...
// React icons
import { AiFillCheckCircle, AiFillCloseCircle, AiOutlineCheckCircle, AiOutlineCloseCircle, AiOutlineDownCircle } from "react-icons/ai";
...
function FrmAttendanceDeviceListing({ compType = 'Masters' }) {
    ...
    const [attendance_device_id, setAtttendenceDeviceId] = useState();

    // Table Data
    const [data, setAttendanceDeviceData] = useState([]);
    ...
    const csvExp = useRef();
    const pdfExpFilters = {};
    var reportName = "Attendance Device Report"
    ...
    useEffect(() => {
        const functionCall = async () => {
            ...
            const availCols = await FnShowAttendenceDeviceRptRecords(reportType);
    ...
    const FnShowAttendenceDeviceRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'hmv_attendance_device_rpt'}`)
    ...
    const infoForUpdate = (attendance_device_id, key) => {
        debugger
        navigate("/Masters/MAttendanceDevice/FrmAttendanceDeviceEntry", { state: { attendance_device_id: attendance_device_id, keyForViewUpdate: key, compType: compType } })
    }

    function viewUpdateDelete(data, key) {
        debugger
        let attendance_device_id = data.attendance_device_id
        setAtttendenceDeviceId(attendance_device_id)
        switch (key) {
            case 'update': infoForUpdate(attendance_device_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(attendance_device_id, 'view'); break;
        }

    }

    async function DeleteAttendenceDevice() {
        debugger
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/HmAttendanceDevice/FnDeleteRecord/${attendance_device_id}/${UserName}`, method)
    ...
    const openForm = () => {
        navigate("/Masters/MAttendanceDevice/FrmAttendanceDeviceEntry", { state: { attendance_device_id: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        ...
        try {
            ...
            const requiredColumns = ['attendance_device_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'attendance_device_id'
            jsonQuery['viewname']['hmv_attendance_device'] = 'hmv_attendance_device';
    ...
    //             nColumn.push({ Headers: "Status", accessor: "status",
    //             Cell: row => (<MDTypography> {row.original.status}  </MDTypography> ),
    //             });
    //             setColumns(nColumn);
    //             setAttendanceDeviceData(responce.content)
    //         } else {
    //             setColumns([]);
    //             setAttendanceDeviceData([])
    //         }
    //         return responce;
    //     } catch (error) {
    //         console.log("error: ", error)
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        ...
        try {
            ...
            if (responce.content.length > 0) {
                ...
                // Check each device's port status
                const updatedContent = await Promise.all(responce.content.map(async device => {
                    const { attendance_device_ip_address, attendance_device_port_no } = device;
                    try {
                        const portCheckResponse = await fetch(`${process.env.REACT_APP_BASE_URL}/api/HmAttendanceDevice/check-port?remoteIp=${attendance_device_ip_address}&port=${attendance_device_port_no}`);
                        const portStatus = await portCheckResponse.text(); 
                        device.status = portStatus.includes('open') ? 'Open' : 'Closed'; 
                    } catch (error) {
                        device.status = 'Error'; // Handle error case
                    }
                    return device;
                }));
                nColumn.push({
                    ...
                    Cell: row => (
                        <span>
                                {row.original.status === 'Open' ? <AiFillCheckCircle  style={{ color: 'green', fontSize: '1.5em' }}/> : row.original.status === 'Closed' ? <AiFillCloseCircle style={{ color: 'red',fontSize: '1.5em' }} /> : <MdError style={{ color: 'orange' }} />}
                        </span>
                    ),
                });
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Attendance Device</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteAttendenceDevice} />

        </>

    )
}
export default FrmAttendanceDeviceListing;