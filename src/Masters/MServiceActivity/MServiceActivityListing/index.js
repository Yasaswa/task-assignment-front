...
import { TbArrowsRight, TbArrowsLeft, TbArrowsDown, TbArrowsUp } from "react-icons/tb";
import { HiOutlineArrowNarrowRight } from "react-icons/hi";
import { HiOutlineArrowNarrowLeft, HiOutlineArrowNarrowUp, HiOutlineArrowNarrowDown } from "react-icons/hi";
...
function FrmMServiceActivityIdListing({ compType = 'Masters' }) {
    const [selectedFilters, setSelectedFilters] = useState([]);
    const [selectedValue, setSelectedValue] = useState("");
    const [prodServiceActivityId, setProdServiceActivityId] = useState();
    ...
    // Table Data
    const [data, setProdServiceActivityData] = useState([]);
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setProdServiceActivityData(responce.content)
            } else {
                setColumns([]);
                setProdServiceActivityData([])
            }
            return responce;
        } catch (error) {
    ...
    const FnShowProdServiceActivityRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'smv_product_service_activity_master_rpt'}`)
            const summaryResponce = await summaryRptApiCall.json();
            console.log("Product Service Activity responce: ", summaryResponce)
    ...
    const infoForUpdate = async (product_service_activity_master_id, key) => {
        sessionStorage.removeItem('dataAddedByCombo')
        navigate("/Masters/ServiceActivity", { state: { prodServiceActivityId: product_service_activity_master_id, keyForViewUpdate: key, compType: compType } })
    }

    function viewUpdateDelete(data, key) {

        let product_service_activity_master_id = data.product_service_activity_master_id
        setProdServiceActivityId(product_service_activity_master_id)
        switch (key) {
            case 'update': infoForUpdate(product_service_activity_master_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(product_service_activity_master_id, 'view'); break;
        }
    }

    async function DeleteProdServiceActivity() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/SmProductServiceActivityMaster/FnDeleteRecord/${prodServiceActivityId}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("Frm Product Service Activity Deleted: ", responce);
            setShow(false)
    ...
    const openForm = () => {
        sessionStorage.removeItem('dataAddedByCombo')
        navigate("/Masters/ServiceActivity", { state: { prodServiceActivityId: 0, keyForViewUpdate: 0, compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['product_service_activity_master_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'product_service_activity_master_id';
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Product Service Activities </label>
                </div>
            ...
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteProdServiceActivity} />

        </>
    )
}

export default FrmMServiceActivityIdListing