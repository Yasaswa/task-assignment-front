...
//changes by ujjwala on 9/1/2024 case no . 2 
function FrmMServiceListing({ compType = 'Masters' }
) {
    ...
    const [product_sr_id, setProductSrId] = useState()
    ...
    const FnShowAllReportRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${reportIdentifierKey}&viewName=${reportIdentifierKey === 'summary' ? 'smv_product_sr_summary_rpt ' : 'smv_product_sr_details_rpt '}`)
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setServiceData(responce.content)
            } else {
                setColumns([]);
                setServiceData([])
            }
    ...
    function viewUpdateDelete(data, key) {
        debugger
        var product_sr_id = data.product_sr_id
        setProductSrId(product_sr_id)
        switch (key) {
            case 'update': infoForUpdate(product_sr_id, 'update'); break;
            // case 'delete': setShow(true); break;
            case 'delete': infoForUpdate(product_sr_id, 'delete'); break;
            case 'view': infoForUpdate(product_sr_id, 'view'); break;
        }
    ...
    const infoForUpdate = async (product_sr_id, key) => {

        navigate("/Masters/MService/FrmMServiceEntry", { state: { productSrId: product_sr_id, keyForViewUpdate: key, compType: compType } })
    ...
    const openForm = () => {
        ...
        // sessionStorage.removeItem('productSrId');
        // sessionStorage.removeItem('productTypeId')
        // sessionStorage.removeItem('category1Id')
        // sessionStorage.removeItem('productSrName');

        // localStorage.removeItem('SrTechnicaldata')
        navigate("/Masters/MService/FrmMServiceEntry", { state: { productSrId: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['product_sr_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'product_sr_id';

            if (reportType === 'summary') {
                reportName = "Service Summary Report"
                jsonQuery['viewname']['viewName'] = 'smv_product_sr_summary'
            } else {
                reportName = "Service Details Report"
                jsonQuery['viewname']['viewName'] = 'smv_product_sr_details'
            }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Service</label>
                </div>
...
export default FrmMServiceListing
