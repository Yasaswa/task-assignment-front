...
function FrmMDepartmentList({ compType = 'Masters' }) {
    ...
    const FnShowDepartmentRptRecords = async () => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_department_rpt'}`)
    ...
    const infoForUpdate = async (department_id, key) => {
        navigate("/Masters/Department", { state: { departmentId: department_id, keyForViewUpdate: key, compType: compType } })
    }
    ...
    function viewUpdateDelete(data, key) {
        ...
        switch (key) {
            case 'update': infoForUpdate(department_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(department_id, 'view'); break;
        }
    }
    ...
    async function DeleteRecord() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/department/FnDeleteRecord/${department_id}/${UserName}
            `, method)
    ...
    const openForm = () => {
        navigate("/Masters/Department", { state: { departmentId: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['department_id']
            ...
            jsonQuery['viewname']['cmv_department'] = 'cmv_department';
            jsonQuery['orderBy']['ORDER BY'] = 'department_id';
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Department Information</label>
                </div>
...
export default FrmMDepartmentList
