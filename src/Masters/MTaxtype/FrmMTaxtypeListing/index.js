...
export default function FrmMTaxtypeList({ compType = 'Masters' }) {
    ...
    // Table Data
    const [data, setTaxtypeData] = useState([]);
    ...
    useEffect(async () => {
        ...
        const availCols = await FnShowTaxtypeRptRecords(reportType);
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setTaxtypeData(responce.content)
            } else {
                setColumns([]);
                setTaxtypeData([])
            }
    ...
    const FnShowTaxtypeRptRecords = async (reportIdentifierKey) => {
        debugger
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_taxtype_rpt'}`)
    ...
    const infoForUpdate = async (taxtype_id, key) => {
        navigate("/Masters/Taxtype", { state: { taxtypeID: taxtype_id, keyForViewUpdate: key, compType: compType } })
    }

    function viewUpdateDelete(data, key) {
        var taxtype_id = data.taxtype_id
        setTaxtypeId(taxtype_id)
        switch (key) {
            case 'update': infoForUpdate(taxtype_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(taxtype_id, 'view'); break;
        }

    }

    async function DeleteTaxtype() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/taxtype/FnDeleteRecord/${taxtypeId}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("Taxtype Deleted: ", responce);
            setShow(false)
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['taxtype_id']
            ...
            jsonQuery['viewname']['cmv_taxation'] = 'cmv_taxtype';
            jsonQuery['orderBy']['ORDER BY'] = 'taxtype_id';
    ...
    const openForm = () => {
        navigate("/Masters/Taxtype", { state: { taxtypeID: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Taxtype Information</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteTaxtype} />
        </>
    )
