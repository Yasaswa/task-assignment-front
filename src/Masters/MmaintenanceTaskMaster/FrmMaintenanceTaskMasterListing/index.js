...
function FrmMaintenanceTaskMasterListing({ compType = 'Masters' }) {
    let storeSelectedValues = [];
    const [recordData, setRecordData] = useState([]);
    const [filterComboBox, setFilterComboBox] = useState([]);
    let reportName = "Maintenance Report";
    ...
    // Table Data
    const [data, setMaintanceData] = useState([]);
    const [columns, setColumns] = useState([]);
    const [selectedFilters, setSelectedFilters] = useState([]);
    const [maintenance_task_master_id, setPropertyId] = useState();
    ...
    const FnShowAllReportRecords = async (reportIdentifierKey) => {
        debugger
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${reportIdentifierKey}&viewName=${reportIdentifierKey === 'summary' ? 'ymv_maintenance_task_master_rpt' : 'ymv_maintenance_task_details_rpt'}`)
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setMaintanceData(responce.content)
            } else {
                setColumns([]);
                setMaintanceData([])
            }
    ...
    function viewUpdateDelete(data, key) {
        debugger
        var maintenance_task_master_id = data.maintenance_task_master_id
        setPropertyId(maintenance_task_master_id)
        switch (key) {
            case 'update': infoForUpdate(maintenance_task_master_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(maintenance_task_master_id, 'view'); break;
    ...
    async function DeleteRecord() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/YmMaintenanceDetails/FnDeleteRecord/${maintenance_task_master_id}/${UserName}/${COMPANY_ID}`, method)
    ...
    const infoForUpdate = async (maintenance_task_master_id, key) => {
        navigate("/Masters/MmaintenanceTaskMaster/FrmMaintenanceTaskMasterEntry", { state: { maintenanceTaskMasterId: maintenance_task_master_id, keyForViewUpdate: key, compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['maintenance_task_master_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'maintenance_task_master_id';

            if (reportType === 'summary') {
                reportName = "maintenance Summary Report"
                jsonQuery['viewname']['viewName'] = 'ymv_maintenance_task_master'
            } else {
                reportName = "maintenance Details Report"
                jsonQuery['viewname']['viewName'] = 'ymv_maintenance_task_details'
            }
    ...
    const openForm = () => {
        navigate("/Masters/MmaintenanceTaskMaster/FrmMaintenanceTaskMasterEntry", { state: { maintenanceTaskMasterId: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Maintenance Task</label>
                </div>
...
export default FrmMaintenanceTaskMasterListing;
