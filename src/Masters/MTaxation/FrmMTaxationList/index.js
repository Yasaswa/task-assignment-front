...
function FrmMTaxationList({ compType = 'Masters' }) {
    const [taxtaionId, setTaxationId] = useState();
    var storeSelectedValues = [];
    const configConstants = ConfigConstants();
    const { COMPANY_ID, UserName, COMPANY_NAME,USER_ACCESS } = configConstants;

    // Table Data
    const [data, setTaxationData] = useState([]);
    ...
    const csvExp = useRef();
    const pdfExpFilters = {};
    var reportName = "Taxation Report"
    ...
    useEffect(async () => {
        ...
        const availCols = await FnShowTaxationRptRecords(reportType);
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setTaxationData(responce.content)
            } else {
                setColumns([]);
                setTaxationData([])
            }
    ...
    const FnShowTaxationRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_taxation_rpt'}`)
    ...
    const infoForUpdate = async (taxation_id, key) => {


        navigate("/Masters/Taxation", { state: { taxationID: taxation_id, keyForViewUpdate: key, compType: compType } })
    }
    function viewUpdateDelete(data, key) {
        var taxation_id = data.taxation_id
        setTaxationId(taxation_id)
        switch (key) {
            case 'update': infoForUpdate(taxation_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(taxation_id, 'view'); break;
        }

    }

    async function DeleteTaxation() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/taxation/FnDeleteRecord/${taxtaionId}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("Taxation Deleted: ", responce);
            setShow(false)
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['taxation_id']
            ...
            jsonQuery['viewname']['cmv_taxation'] = 'cmv_taxation';
            jsonQuery['orderBy']['ORDER BY'] = 'taxation_id';
    ...
    const openForm = () => {
        navigate("/Masters/Taxation", { state: { taxationID: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Taxation Information</label>
                </div>
            ...
            <DeleteModal
                show={show} closeModal={handleClose} deleteRecord={DeleteTaxation} />
        </>

    )
}

export default FrmMTaxationList