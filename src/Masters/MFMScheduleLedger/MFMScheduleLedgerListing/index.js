...
function MFMScheduleLedgerListing({ compType = 'Masters' }) {

    const configConstants = ConfigConstants();
    const { COMPANY_ID, UserName, COMPANY_NAME,USER_ACCESS } = configConstants;

    const [schedule_ledger_id, setschedule_ledger_id] = useState();
    var storeSelectedValues = [];

    // Table Data
    const [data, setScheduleLedgerData] = useState([]);
    ...
    const pdfExpFilters = {};
    var reportName = "ScheduleLedger Report"
    ...
    useEffect(async () => {
        ...
        const availCols = await FnShowScheduleledgerRptRecords(reportType);
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setScheduleLedgerData(responce.content)
            } else {
                setColumns([]);
                setScheduleLedgerData([])
            }
            return responce;
        } catch (error) {
    ...
    const FnShowScheduleledgerRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'fmv_schedule_ledger_rpt'}`)
    ...
    const infoForUpdate = async (schedule_ledger_id, key) => {
        navigate("/Masters/MFMScheduleLedgerEntry", { state: { scheduleLedgerId: schedule_ledger_id, keyForViewUpdate: key, compType: compType } })
    }

    function viewUpdateDelete(data, key) {
        debugger
        var schedule_ledger_id = data.schedule_ledger_id
        setschedule_ledger_id(schedule_ledger_id)
        switch (key) {
            case 'update': infoForUpdate(schedule_ledger_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(schedule_ledger_id, 'view'); break;
        }
    ...
    async function DeleteSchedule() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/scheduleledger/FnDeleteRecord/${schedule_ledger_id}/${COMPANY_ID}/${UserName}`

                , method)
            const responce = await deleteApiCall.json();
            console.log("schedule Deleted: ", responce);
            setShow(false)
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['schedule_ledger_id']
            ...
            jsonQuery['viewname']['fmv_schedule_ledger'] = 'fmv_schedule_ledger';
            jsonQuery['orderBy']['ORDER BY'] = 'schedule_ledger_id';
    ...
    const openForm = () => {
        navigate("/Masters/MFMScheduleLedgerEntry", { state: { scheduleLedgerId: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Schedule Ledger</label>
                </div>
            ...
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteSchedule} />
        </>
    )
...
export default MFMScheduleLedgerListing