...
function MFrmProductionElectricalMeterListing({ compType = 'Masters' }) {

    const configConstants = ConfigConstants();
    const { COMPANY_ID, UserName, COMPANY_BRANCH_ID, COMPANY_NAME, USER_ACCESS } = configConstants;

    let storeSelectedValues = [];
    const [recordData, setrecordData] = useState([])
    const [filterComboBox, setFilterComboBox] = useState([]);
    let reportName = "Electrical Meter Report";
    ...
    // Table Data
    const [data, setElectricalMeter] = useState([]);
    const [columns, setColumns] = useState([]);


    const [prod_electrical_meter_id, setPropertyId] = useState();
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setElectricalMeter(responce.content)
            } else {
                setColumns([]);
                setElectricalMeter([])
            }
    ...
    const FnShowAllReportRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${reportIdentifierKey}&viewName=${reportIdentifierKey === 'summary' ? 'xmv_production_electrical_meter_rpt' : 'xmv_production_electrical_meter_machine_mapping_rpt'}`)
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['prod_electrical_meter_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'prod_electrical_meter_id';

            if (reportType === 'summary') {
                reportName = "Electrical Summary Report"
                jsonQuery['viewname']['viewName'] = 'xmv_production_electrical_meter'
            } else {
                reportName = "Electrical Details Report"
                jsonQuery['viewname']['viewName'] = 'xmv_production_electrical_meter_machine_mapping'
            }
    ...
    function viewUpdateDelete(data, key) {
        debugger
        var prod_electrical_meter_id = data.prod_electrical_meter_id
        setPropertyId(prod_electrical_meter_id)
        switch (key) {
            case 'update': infoForUpdate(prod_electrical_meter_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(prod_electrical_meter_id, 'view'); break;
    ...
    async function DeleteRecord() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/XmProductionElectricalMeter/FnDeleteRecord/${prod_electrical_meter_id}/${UserName}/${COMPANY_ID}`, method)
    ...
    const infoForUpdate = async (prod_electrical_meter_id, key) => {
        navigate("/Masters/MProductionElectricalMeter/MFrmProductionElectricalMeterEntry", { state: { prod_electrical_meter_id: prod_electrical_meter_id, keyForViewUpdate: key, compType: compType } })
    }
    ...
    const openForm = () => {
        navigate("/Masters/MProductionElectricalMeter/MFrmProductionElectricalMeterEntry", { state: { prod_electrical_meter_id: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    const reload = () => {
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Electrical Meter Master</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteRecord} />

        </>
    )
}

export default MFrmProductionElectricalMeterListing;
