...
export default function FrmPropertyListing({ compType = 'Masters' }) {
    ...
    const pdfExpFilters = {};
    var reportName = "Property Report"
    ...
    useEffect(() => {
        const functionCall = async () => {
            ...
            const availCols = await FnShowPropertyRptRecords(reportType);
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setPropertyData(responce.content)
            } else {
                setColumns([]);
                setPropertyData([])
            }
    ...
    const FnShowPropertyRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'amv_properties_rpt'}`)
    ...
    const infoForUpdate = async (property_id, key) => {
        navigate("/Masters/FrmPropertyEntry", { state: { propertyid: property_id, keyForViewUpdate: key, compType: compType } })
    }
    ...
    function viewUpdateDelete(data, key) {
        ...
        switch (key) {
            case 'update': infoForUpdate(property_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(property_id, 'view'); break;
        }
    ...
    async function DeleteProperty() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/properties/FnDeleteRecord/${property_id}/${COMPANY_ID}/${UserName}`

                , method)
            const responce = await deleteApiCall.json();
            console.log("property Deleted: ", responce);
            setShow(false)
    ...
    const openForm = () => {
        navigate("/Masters/FrmPropertyEntry", { state: { propertyid: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    //     jsonQuery['viewname']['amv_properties'] = 'amv_properties';
    //     jsonQuery['orderBy']['ORDER BY'] = 'property_id';
    //     let executeQuery = JSON.stringify(jsonQuery)
    //     return executeQuery;
    // }
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['property_id']
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Property Master</label>
                </div>
            ...
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteProperty} />
        </>
    )
