...
function FrmMHMDeductionHeadsListing({ compType = 'Masters' }) {

    const configConstants = ConfigConstants();
    const { UserName, COMPANY_ID, COMPANY_NAME, USER_ACCESS } = configConstants;

    var storeSelectedValues = [];
    //session ID 
    const [deduction_headsID, setdeduction_headsID] = useState();

    //table Feilds
    const [data, setdeductionHeadsData] = useState([]);
    ...
    var reportName = "Deduction Heads Report"
    ...
    useEffect(async () => {
        ...
        const availCols = await showdeductionHeadsIDReportRecords(reportType);
        await fetchFilteredData(PageCurrent, entriesPerPage, availCols)

    }, [compType])

    const showdeductionHeadsIDReportRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'hmv_deduction_heads_rpt'}`)
    ...
    const infoForUpdate = async (deduction_headsID, key) => {
        navigate("/Masters/MHMdeductionHeads/FrmMHMdeductionHeadsEntry", { state: { deduction_headsIDs: deduction_headsID, keyForViewUpdate: key, compType: compType } })
    }

    function viewUpdateDelete(data, key) {
        var deduction_headsID = data.deduction_heads_id
        setdeduction_headsID(deduction_headsID)
        switch (key) {
            case 'update': infoForUpdate(deduction_headsID, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(deduction_headsID, 'view'); break;
        }
    }

    const openForm = () => {
        navigate("/Masters/MHMdeductionHeads/FrmMHMdeductionHeadsEntry", { state: { deduction_headsIDs: 0, keyForViewUpdate: '', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['deduction_heads_id', 'deduction_head_version']
            ...
            jsonQuery['viewname']['viewName'] = 'hmv_deduction_heads'
            jsonQuery['orderBy']['ORDER BY'] = "deduction_heads_id";
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setdeductionHeadsData(responce.content)
            } else {
                setColumns([]);
                setdeductionHeadsData([])
            }
            return responce;
        } catch (error) {
            console.log("error: ", error)
    ...
    async function DeletedeductionHeads() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/HmDeductionHeads/FnDeleteRecord/${deduction_headsID}/${COMPANY_ID}/${UserName}`, method)
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Deduction Heads</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeletedeductionHeads} />
...
export default FrmMHMDeductionHeadsListing          
