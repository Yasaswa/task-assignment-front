...
function FrmBeamInwardsListing({ compType = 'Masters' }) {
  ...
  let reportName = "Beam Inwards Listing";

  // Popup Fields
  const [show, setShow] = useState(false);
  const handleClose = () => setShow(false);

  const [beam_inwards_id, setSalaryRuleId] = useState();
  ...
  const fetchFilteredData = async (page, size, availCols) => {
    ...
    try {
      ...
      if (responce.content.length > 0) {
        // This for pagination 
        const access_control = FnCheckUserAccess() || { all_access: true, read_access: true, modify_access: true, delete_access: true }; // Setting default access control when USER_ACCESS is undefined
              ...
              Cell: row => (
                <div style={{ display: "flex" }}>
                  <Tooltip title="view" placement="top">
                    <MDTypography className={`erp-view-btn ${row.original.property_value === 'IN-HOUSE' ? `d-none` : 'dispaly'}`} >
                  ...
                  {compType === 'Register' ?
                    null : <>
                      <Tooltip title="Edit" placement="top">
                        <MDTypography className={`erp-view-btn ${row.original.property_value === 'IN-HOUSE' || row.original.beam_status_desc == 'Completed' ? `d-none` : 'dispaly'}`} >
  ...
  const FnShowAllReportRecords = async (reportIdentifierKey) => {
    try {
      const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'xtv_beam_inwards_table_rpt'}`)
  ...
  const submitQuery = async (availCols) => {
    try {
      ...
      jsonQuery['viewname']['viewName'] = 'xtv_beam_inwards_table';
      jsonQuery['orderBy']['ORDER BY'] = 'beam_inwards_id';
  ...
  const openForm = () => {
    navigate("/Masters/MBeamInwards/MBeamInwardsEntry", { state: { customerId: 0, keyForViewUpdate: 'Add', beamInwardsDate: today() } })
  }


  const infoForUpdate = async (customer_id, key, beam_inwards_date) => {
    navigate("/Masters/MBeamInwards/MBeamInwardsEntry", { state: { customerId: customer_id, keyForViewUpdate: key, beamInwardsDate: beam_inwards_date } })
  }



  function viewUpdateDelete(rowdata, key) {
    let customerId = rowdata.customer_id
    setCustomerId(customerId);
    var beam_inwards_date = rowdata.beam_inwards_date
    switch (key) {
      case 'update': infoForUpdate(customerId, 'update', beam_inwards_date); break;
      case 'delete': setShowModal(true); break;
      case 'view': infoForUpdate(customerId, 'view', beam_inwards_date); break;
    }
  }
  ...
  const [beamdIds, setBeamIds] = useState([]);

  const openModalForIssue = () => {
    console.log("customerId :" + customerId);
    return (
      <Modal show={showModal} backdrop="static" keyboard={false} centered>
        <span>
          <button
            type="button" className="erp-modal-close btn-close" aria-label="Close"
        ...
        <Modal.Body className="erp_modal_body">
          ...
          <h6>Beam No's to be deleted...!!</h6>
          <h5>
            {[...new Set(
              data
                .filter((d) => d.customer_id === customerId && d.beam_status_desc === "Empty")
                .map((d) => d.beam_inwards_id) // Ensure uniqueness only on `beam_inwards_id`
            )].map((beamId) => {
              const beamData = data.find((d) => d.beam_inwards_id === beamId); // Find corresponding data
              return (
                <label key={beamId} style={{ display: "block" }}>{' '}
                  <input
                    type="checkbox"
                    name="beamNoIds"
                    value={beamId}
                    onChange={fnGetBeamId}
                  />{beamData?.beam_inward_type}
                </label>
              );
            })}
          </h5>
          <h6>You Really want to delete ?</h6>

          <Modal.Footer className="erp-modal-footer">
            <Button variant="success" className="erp-gb-button" disabled={beamdIds.length === 0} onClick={() => DeleteRecords()}>
              Yes
            </Button>

            <Button variant="success" className="erp-gb-button ms-2" onClick={() => closeModal()}> No</Button>
          </Modal.Footer>
        </Modal.Body>
      </Modal>
    );
  ...
  const fnGetBeamId = (e) => {
    // debugger
    const beamId = e.target.value;
    setBeamIds((prevBeamIds) => {
      if (prevBeamIds.includes(beamId)) {
        // If already selected, remove it (uncheck scenario)
        return prevBeamIds.filter(id => id !== beamId);
      } else {
        // If not selected, add it (check scenario)
        return [...prevBeamIds, parseInt(beamId)];
      }
    });
  };
  ...
  const DeleteRecords = async () => {
    ...
    setIsLoading(true)
    setShowModal(false);
    let json = { commonIds: { 'USERNAME': UserName, 'company_id': COMPANY_ID, 'customer_id': customerId }, beamIds: beamdIds };
    ...
    let response = await fetch(`${process.env.REACT_APP_BASE_URL}/api/XtBeamInwards/FnDeleteBeamInwardsData`, requestOptions);
    closeModal();
    setIsLoading(false)
    ...
    <>
      ...
      <DashboardLayout>
        <div className='main_heding'>
          <label className='erp-form-label-lg'>Beam Inwards</label>
        </div>
      ...
      </DashboardLayout > {openModalForIssue()}
      {/* <DeleteModal show={show} closeModal={handleClose} deleteRecord={} /> */}
    </>
  )
}

export default FrmBeamInwardsListing
