...
import { TbArrowsDown, TbArrowsLeft, TbArrowsRight, TbArrowsUp } from 'react-icons/tb';
import { HiOutlineArrowNarrowDown, HiOutlineArrowNarrowLeft, HiOutlineArrowNarrowRight, HiOutlineArrowNarrowUp } from 'react-icons/hi';
...
function FrmPlantlist({ compType = 'Masters' }) {
    ...
    useEffect(() => {
        ...
        const rptApiCall = async () => {
            const availCols = await FnShowPlantRptRecords(reportType);
            await fetchFilteredData(PageCurrent, entriesPerPage, availCols)
        }
        rptApiCall()
    }, [compType])

    //Function for fetch all records initially
    const FnShowPlantRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_plant_rpt'}`)
    ...
    // Function for fetch records for export
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['plant_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = "plant_id";
            jsonQuery['viewname']['viewName'] = 'cmv_plant'
    ...
    // Function for Add Button Click and open add form
    const openForm = () => {
        navigate("/Masters/FrmPlantEntry", { state: { PlantId: 0, keyForViewUpdate: 'Add', compType: compType } })
    }

    // Function for open info form for edit and view
    const infoForUpdate = async (plant_id, key) => {
        navigate("/Masters/FrmPlantEntry", { state: { PlantId: plant_id, keyForViewUpdate: key, compType: compType } })
    }

    //Function for open options for edit/view/delete
    function viewUpdateDelete(data, key) {
        var plant_id = data.plant_id
        setPlantId(plant_id)
        switch (key) {
            case 'update': infoForUpdate(plant_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(plant_id, 'view'); break;
        }
    ...
    async function DeleteRecord() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/CmPlant/FnDeleteRecord/${plantId}/${COMPANY_ID}/${UserName}`, method)
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Plant Information</label>
                </div>
