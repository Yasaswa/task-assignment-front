...
function FrmLeavesApplicationListing({ compType = 'Transactions' }) {
    let storeSelectedValues = [];

    const configConstants = ConfigConstants();
    const { COMPANY_ID, COMPANY_NAME, UserName, USER_ACCESS, UserId, DEPARTMENT_ID, } = configConstants;
    const [punch_code, setPunchCode] = useState('');
    const [requestedFromDt, setRequestedFromDate] = useState();
    const [requestedToDt, setRequestedToDate] = useState();

    const [leavesId, setLeavesId] = useState();
    const [recordData, setRecordData] = useState([]);
    const [filterComboBox, setFilterComboBox] = useState([]);
    const [reportType, setReportType] = useState('summary');
    let reportName = "Leaves Application Report";
    ...
    const [employeeDetails, setEmployeeListOption] = useState([])
    //useRef
    const pdfExp = useRef();
    ...
    // Table Data
    const [data, setLeaveApplication] = useState([]);
    ...
    const FnShowAllReportRecords = async (reportIdentifierKey) => {
        debugger
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'hmv_leaves_applications_rpt'}`)
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        ...
        try {
            ...
            if (responce.content.length > 0) {
                ...
                globalQuery.conditions.push({ field: "username", operator: "=", value: UserName });
                globalQuery.conditions.push({ field: "is_delete", operator: "=", value: 0 });
                const employeeDetailsApiCall = await comboDataAPiCall.current.fillFiltersCombo(globalQuery);
                setEmployeeListOption(employeeDetailsApiCall);
                            ...
                            Cell: row => (
                                ...
                                <div style={{ display: "flex" }}>
                                    <Tooltip title="View" placement="top">
                                        <MDTypography className={`erp-view-btn ${access_control.all_access || access_control.read_access ? 'display' : 'd-none'}`} >
                                            <AiFillEye className="erp-view-btn" onClick={e => viewUpdateDelete(row.original, 'view', access_control)} />
                                        </MDTypography>
                                    </Tooltip>
                                    {compType === 'Register' ? null : <>
                                        {/* <Tooltip title={row.original.leave_status === 'Approved' ? 'Leave Approved' : row.original.leave_status === 'Rejected' ? 'Leave Rejected' : row.original.leave_status === 'Cancelled' ? 'Leave Status Cancel' : 'Approved'} placement="top">
                                                <MDTypography className={`erp-view-btn ${access_control.all_access || access_control.read_access ? 'display' : 'd-none'}`} >
                                                    <BsFillCheckCircleFill className="erp-view-btn" onClick={e => {
                                                        if (row.original.leave_status !== 'Approved' && row.original.leave_status !== 'Rejected' && row.original.leave_status !== 'Cancelled') {
                                                            viewUpdateDelete(row.original, 'approve')
                                                        }
                                                    }} disabled={row.original.leave_status === 'Approved' || row.original.leave_status === 'Rejected' || row.original.leave_status === 'Cancelled'} />
                                                </MDTypography>
                                            </Tooltip> */}
                                        {row.original.leave_status !== 'Approved' && row.original.reporting_to === parseInt(UserId) && (
                                            <Tooltip title={row.original.leave_status === 'Approved' ? 'Leave Approved' : row.original.leave_status === 'Rejected' ? 'Leave Rejected' : row.original.leave_status === 'Cancelled' ? 'Leave Status Cancel' : 'Approved'} placement="top">
                                                <MDTypography className={`erp-view-btn ${employeeDetailsApiCall[0] &&
                                                    (employeeDetailsApiCall[0].department_name === 'Human Resource Management' || row.original.reporting_to === employeeDetailsApiCall[0].field_id || (row.original.applied_by_id.includes(',') ? row.original.applied_by_id.split(',').includes(String(UserId)) : row.original.applied_by_id === String(UserId))) ? 'display' : 'd-none'}`} >
                                                    <BsFillCheckCircleFill className="erp-view-btn" onClick={e => {
                                                        if (row.original.leave_status !== 'Approved' && row.original.leave_status !== 'Rejected' && row.original.leave_status !== 'Cancelled') {
                                                            viewUpdateDelete(row.original, 'approve', access_control)
                                                        }
                                                    }} disabled={row.original.leave_status === 'Approved' || row.original.leave_status === 'Rejected' || row.original.leave_status === 'Cancelled'} />
                                                </MDTypography>
                                            </Tooltip>)}
                                        {(row.original.created_by === UserName || row.original.reporting_to === parseInt(UserId)) && row.original.leave_status !== 'Approved' && (
                                            <Tooltip title={row.original.leave_status === 'Approved' ? 'Leave Approved' : row.original.leave_status === 'Rejected' ? 'Leave Rejected' : row.original.leave_status === 'Cancelled' ? 'Leave Status Cancelled' : 'Cancel'} placement="top">
                                                <MDTypography className="erp-view-btn" >
                                                    < MdOutlineCancel style={{ color: '#F31F1F' }} className={`erp-view-btn ${access_control.all_access || access_control.read_access ? 'display' : 'd-none'}`}
                                                        onClick={e => {
                                                            if (row.original.leave_status !== 'Approved' && row.original.leave_status !== 'Rejected' && row.original.leave_status !== 'Cancelled') {
                                                                viewUpdateDelete(row.original, 'cancel', access_control)
                                                            }
                                                        }} disabled={row.original.leave_status === 'Approved' || row.original.leave_status === 'Rejected' || row.original.leave_status === 'Cancelled'} />

                                                </MDTypography>
                                            </Tooltip>)}
                                        {row.original.created_by === UserName && row.original.leave_status !== 'Approved' && (
                                            <Tooltip title={row.original.leave_status === 'Approved' ? 'Leave Approved' : row.original.leave_status === 'Rejected' ? 'Leave Rejected' : row.original.leave_status === 'Cancelled' ? 'Leave Status Cancel' : 'Edit'} placement="top">
                                                <MDTypography className="erp-view-btn" >
                                                    <MdModeEdit className={`erp-edit-btn ${(access_control.all_access || access_control.read_access) && row.original.approval_status !== 'Approved' ? 'display' : 'd-none'}`} onClick={e => {
                                                        if (row.original.leave_status !== 'Approved' && row.original.leave_status !== 'Rejected' && row.original.leave_status !== 'Cancelled') {
                                                            viewUpdateDelete(row.original, 'update', access_control)
                                                        }
                                                    }} disabled={row.original.leave_status === 'Approved' || row.original.leave_status === 'Rejected' || row.original.leave_status === 'Cancelled'} />
                                                </MDTypography>
                                            </Tooltip>)}
                                        {row.original.created_by === UserName && row.original.leave_status !== 'Approved' && (
                                            <Tooltip title={row.original.leave_status === 'Approved' ? 'Leave Approved' : row.original.leave_status === 'Rejected' ? 'Leave Rejected' : row.original.leave_status === 'Cancelled' ? 'Leave Status Cancel' : 'Delete'} placement="top">
                                                <MDTypography className="erp-view-btn" >
                                                    <MdDelete className={`erp-delete-btn ${(access_control.all_access || access_control.read_access) && row.original.approval_status !== 'Cancelled' ? 'display' : 'd-none'}`} onClick={e => {
                                                        if (row.original.leave_status !== 'Approved' && row.original.leave_status !== 'Rejected' && row.original.leave_status !== 'Cancelled') {
                                                            viewUpdateDelete(row.original, 'delete', access_control)
                                                        }
                                                    }} disabled={row.original.leave_status === 'Approved' || row.original.leave_status === 'Rejected' || row.original.leave_status === 'Cancelled'} />
                                                </MDTypography>
                                            </Tooltip>)}
                ...
                setColumns(nColumn);
                setLeaveApplication(responce.content)
            } else {
                setColumns([]);
                setLeaveApplication([])
            }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['leaves_transaction_id']
            ...
            // jsonQuery['additionalParam']['leave_type_id'] != "11";

            jsonQuery['orderBy']['ORDER BY'] = 'leaves_transaction_id'
            jsonQuery['viewname']['hmv_leaves_applications'] = 'hmv_leaves_applications';
            if (UserId !== "1" && UserId !== 1 && UserId !== "3" && UserId !== 3) {
            ...
            jsonQuery['whereQuery']['leave_type_id'] = '11';
            jsonQuery['operators']['leave_type_id'] = '!=';
    ...
    function viewUpdateDelete(data, key, access_control) {
        debugger
        let modules_forms_id = access_control.modules_forms_id;
        var leaves_transaction_id = data.leaves_transaction_id
        setLeavesId(leaves_transaction_id)
        setPunchCode(data.punch_code)
        setRequestedFromDate(data.leaves_requested_from_date)
        setRequestedToDate(data.leaves_requested_to_date)

        if (key === 'view' || key === "update") {
            infoForUpdate(leaves_transaction_id, key, modules_forms_id);
        }
        if (data.leave_status !== 'Approved' && data.leave_status !== 'Rejected' && data.leave_status !== 'Cancelled') {
            switch (key) {
                case 'update': infoForUpdate(leaves_transaction_id, 'update', modules_forms_id); break;
                case 'approve': infoForUpdate(leaves_transaction_id, 'approve', modules_forms_id); break;
                case 'delete': setShow(true); break;
                case 'cancel': infoForUpdate(leaves_transaction_id, 'cancel', modules_forms_id); break;
                case 'add': infoForUpdate(leaves_transaction_id, 'add', modules_forms_id); break;
                default: break;
            }
        }
    }
    ...
    const deleteRecords = async () => {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/HmLeavesApplications/FnDeleteRecord/${punch_code}/${requestedFromDt}/${requestedToDt}/${UserName}`, method)
    ...
    const infoForUpdate = async (leaves_transaction_id, key, modules_forms_id) => {
        if (key === 'view') {
            navigate("/Masters/MLeavesApplication/FrmLeavesApplicationEntry", { state: { leavesId: leaves_transaction_id, keyForViewUpdate: 'view', compType: compType, modules_forms_id: modules_forms_id } })
        } else {
            navigate("/Masters/MLeavesApplication/FrmLeavesApplicationEntry", { state: { leavesId: leaves_transaction_id, keyForViewUpdate: key, compType: compType, modules_forms_id: modules_forms_id } })
        }
    }
    ...
    const openForm = () => {
        ...
        navigate("/Masters/MLeavesApplication/FrmLeavesApplicationEntry", { state: { leavesId: 0, keyForViewUpdate: 'add', compType: compType, modules_forms_id: access_control.modules_forms_id } })
    }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Leaves Application</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={deleteRecords} />
        </>
    )
}

export default FrmLeavesApplicationListing;
