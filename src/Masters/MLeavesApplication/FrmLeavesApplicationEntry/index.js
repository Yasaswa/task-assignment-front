...
function FrmLeavesApplicationEntry() {
    ...
    function convertDate(dateString) {
        ...
        let month = String(dateObj.getMonth() + 1).padStart(2, '0'); // Months are zero-indexed in JavaScript
        ...
        try {
            ...
            if (keyForViewUpdate === "add") {
                ...
                if (getAppliedByApiCall.length > 0) {
                    //getAppliedByApiCall = getAppliedByApiCall?.map(prop => ({ ...prop, value: prop.field_id, label: prop.field_name, punching_code: prop.old_employee_code, reporting_to: prop.reporting_to}));
                ...
                //     ...getAppliedByApiCall.map((appliedBy) => ({ ...appliedBy, value: appliedBy.field_id, label: appliedBy.field_name })),
                // ];
            ...
            const filteredLeaveTypes = leaveTypeApiCall.filter(leaveType => leaveType.field_name !== "Official Journey");
            setLeaveTypesAllOption(filteredLeaveTypes);
        ...
        switch (key) {
            case 'employee_type':
                ...
                if (employeeType !== "") {
                    ...
                    let getAppliedByApiCall = await comboDataAPiCall.current.fillFiltersCombo(globalQuery);
                    if (getAppliedByApiCall.length > 0) {
                        // getAppliedByApiCall = getAppliedByApiCall?.map(prop => ({ ...prop, value: prop.field_id, label: prop.field_name, punching_code: prop.old_employee_code }));
                ...
                if (employeeCode !== "" && employeetype !== "") {
                    ...
                    if (employeeCode.length >= 3) {
                        ...
                        if (serachemployeeCode) {
                            ...
                            // setAppliedById(serachemployeeCode.field_id);
                            //****************************************** */
                            if (keyForViewUpdate === "add") {
                                setReportingTo(serachemployeeCode.reporting_to);
                            ...
                            //****************************************** */
                            // setEmployeeCode(serachemployeeCode.employee_code);
                        ...
                        // Handle case where the input length is less than 3 characters
                        // setEmployeeCode('');
                        setEmployeeName('');
                        setReportingTo('');
                        setDepartmentId('');
            ...
            case 'empl_code':
                ...
                // const selectedEmployeecode = document.getElementById('cmb_employee_id').options[document.getElementById('cmb_employee_id').selectedIndex].textContent;
            ...
            case 'employee_name':
                ...
                if (selectedEmployee) {
                    ...
                    // setAppliedById(selectedEmployee.field_id);
                    //****************************************** */
                    if (keyForViewUpdate === "add") {
            ...
            case 'employee_baseDeparmentAndType':
                ...
                // const selectedEmployeeField = document.getElementById('cmb_employee_id').options[document.getElementById('cmb_employee_id').selectedIndex].textContent;
            ...
            case 'leave_type':
                ...
                if (leaveAppliedDays !== "" && tempClosingBalance !== null) {
                    if(leaveType === "10" || leaveType === 10){
                        $('#error_leaves_applied_days').hide();
                    }else if (leaveAppliedDays > tempClosingBalance ) {
            ...
            case 'leave_To_date':
                ...
                if (document.getElementById('txt_employee_code').value !== "") {
                    ...
                    if (count !== 0 && keyForViewUpdate === 'add') {
                        ...
                        $(`#error_dt_leaves_requested_to_date`).hide();
                        // const apiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/HmLeavesApplications/FnShowCheckSandwich/${weeklyoff}/${leaveRequestedFromDate}/${leaveRequestedToDate}`)
                        // const response = await apiCall.json();
                        // let sandwich = response.isSandwiched;
                        // if (sandwich === true) {
                        //     setLeaveSandwich('Yes');
                        // } else {
                        //     setLeaveSandwich('No'); 
                        // }
    ...
    const FnFillEmployeeLeavesBalance = async () => {
        ...
        try {
            if (document.getElementById('txt_employee_code').value !== "" && employeetype !== "") {
                ...
                // globalQuery.conditions.push({ field: "E.employee_code", operator: "=", value: emplCode });
                ...
                const filteredLeaveTypes = getLeavesDetails.filter(leaveType => leaveType.leave_type_name !== "Official Journey");
                setLeavesApplicationData(filteredLeaveTypes)
    ...
    const validatedatetimeFields = async (key, date) => {
        ...
        switch (key) {
            case 'leave_date':
                ...
                break;
            case 'leave_Fromdate':
                // const startApproveDateTime = date instanceof Date ? date : parseDate(date);
                // const endApproveDate = document.getElementById('dt_leaves_approved_to_date').value;
                //    const endApproveDateTimes = parseDate(endApproveDate);
    ...
    const FnCheckUpdateResponce = async () => {
        ...
        try {
            ...
            //     let tempClosingBalance = null;
            //     resetGlobalQuery();
            //     globalQuery.columns = ['coff_days'];
            //     globalQuery.table = "ht_attendance_monthly";
            //     globalQuery.conditions.push({ field: "employee_code", operator: "=", value: employee_code });
            //     // globalQuery.conditions.push({ field: "employee_type", operator: "=", value: "Staff" });
            //     globalQuery.conditions.push({ field: "is_delete", operator: "=", value: 0 });

            //     const coffAPICall = await comboDataAPiCall.current.fillFiltersCombo(globalQuery);
            //     if (coffAPICall) {
            //         tempClosingBalance = coffAPICall.reduce((sum, item) => sum + item.coff_days, 0);
            //     }
            //     setClosingBalance(tempClosingBalance);
            // }
    ...
    const validateApproval = () => {
        ...
        if (!leaveType || leaveType === "") {
            $(`#error_txt_leave_type_id`).text(`Please select at least one option..`);
            $(`#error_txt_leave_type_id`).show();
            isValid = false;
        } else {
            $(`#error_txt_leave_type_id`).hide();
        }
    ...
    const leavesApplicationData = async () => {
        debugger
        const leaveAppliedDays = parseFloat(document.getElementById('leaves_applied_days').value);
        const ClosingBalance = (txt_leave_type_id === "10" || txt_leave_type_id == 10) ? leaveAppliedDays : parseFloat(document.getElementById('closing_balance').value);
                                            ...
                                            {/* <div className="col-12 col-md-5 pt-md-0 pt-3 ps-md-1">
                                                <select id="cmb_employee_id" className="form-select form-select-sm" value={cmb_employee_id} onChange={e => { FnComboOnchange('employee_name'); setEmployeeName(e.target.value); validateErrorMsgs() }} maxLength="20">
                                                    ...
                                                    {employeeAppliedBy?.map(employeename => (
                                                        <option value={employeename.field_id} empCode={employeename.employee_code} punching_code={employeename.old_employee_code}>{employeename.field_name}</option>
                                                    ))}
                                                </select>
                                ...
                                <div className="col-sm-4 erp_form_col_div">
                                    <div className="col-sm-12 erp_form_col_div">
                                        {/* <div className="row">
                                            ...
                                            <div className="col">
                                                <select id="applied_by_id" className="form-select form-select-sm" value={applied_by_id} onChange={e => { setAppliedById(e.target.value); validateErrorMsgs(); }} disabled maxLength="255">
                                                    <option value="" disabled>Select</option>
                                                    {appliedAllEmployee?.map(employeename => (
                                        ...
                                        <div className="row">
                                            <div className="col-sm-4">
                                                <Form.Label className="erp-form-label">Work Handover To </Form.Label>
                                            </div>
                                            <div className="col">
                                                <select id="txt_work_handover_id" className="form-select form-select-sm" value={txt_work_handover_id} onChange={e => { setWorkHandoverId(e.target.value); validateErrorMsgs(); }} optional="optional" maxLength="255">
                                ...
                                <form id="leaveForm">
                                    ...
                                    <div className="row erp_transporter_div ">
                                        <div className="col-sm-4 erp_form_col_div">
                                            ...
                                            <div className="col">
                                                <Form.Control type="date" id="dt_leaves_requested_from_date" className="erp_input_field" value={dt_leaves_requested_from_date} onChange={e => { calculateAppliedDays(); FnComboOnchange('leave_To_date'); setLeavesRequestedFromDate(e.target.value); }} onBlur={e => { calculateAppliedDays(); }}  />
                                                ...
                                                <div className="row">
                                                    ...
                                                    <div className="col-sm-3">
                                                        <Form.Control type="number" id="leaves_applied_days" className="erp_input_field" value={leaves_applied_days} onChange={(e) => { FnComboOnchange('leaveAppliedDays', e); validateError() }} disabled />
                                                    </div>
                                        ...
                                        <div className="col-sm-4 erp_form_col_div">
                                            <div className="col-sm-12 erp_form_col_div">
                                               {txt_leave_type_id !== "10" && ( 
                                                <div className="row" hidden={txt_leave_type_id === "10" || txt_leave_type_id === 10}>
                                                    <div className="col-sm-4">
                                            ...
                                            <div className="col">
                                                <Form.Control type="date" id="dt_leaves_requested_to_date" className="erp_input_field" value={dt_leaves_requested_to_date}
                                                    ...
                                                    onBlur={e => { FnComboOnchange('leave_To_date'); FnComboOnchange('leave_type'); }} />
