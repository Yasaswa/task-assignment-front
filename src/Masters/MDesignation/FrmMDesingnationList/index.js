...
export default function FrmMDesignationList({ compType = 'Masters' }) {
    ...
    const pdfExpFilters = {};
    var reportName = "Designation Report"
    ...
    useEffect(async () => {
        ...
        const availCols = await FnShowDesignationRptRecords(reportType);
        await fetchFilteredData(PageCurrent, entriesPerPage, availCols)
    }, [compType])

    const FnShowDesignationRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_designation_rpt'}`)
    ...
    const openForm = () => {
        ...
        navigate("/Masters/Designation", { state: { designationId: 0, keyForViewUpdate: 'Add', compType: compType } })
    }

    const infoForUpdate = (data, key) => {
        navigate("/Masters/Designation", { state: { designationId: data.designation_id, keyForViewUpdate: key, compType: compType } })
    }
    ...
    async function DeleteDesignation() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/designation/FnDeleteRecord/${designationId}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("Designation Deleted: ", responce);
            setShow(false)
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['designation_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'designation_id';
            jsonQuery['viewname']['cmv_designation'] = 'cmv_designation';
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Designation Information</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteDesignation} />
        </>
    )
}