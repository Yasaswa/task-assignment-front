...
function FrmPaymentTemrsListing({ compType = 'Masters' }) {
    ...
    // Table Data
    const [data, setPaymentTermsData] = useState([]);
    ...
    const pdfExpFilters = {};
    var reportName = "PaymentTerms Report"
    ...
    useEffect(async () => {
        ...
        const availCols = await FnShowPaymentTermsRptRecords(reportType);
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setPaymentTermsData(responce.content)
            } else {
                setColumns([]);
                setPaymentTermsData([])
            }
    ...
    const FnShowPaymentTermsRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_payment_terms_rpt'}`)
    ...
    const infoForUpdate = async (payment_terms_id, key) => {
        navigate("/Masters/FrmPaymentTermsEntry", { state: { paymentTermsID: payment_terms_id, keyForViewUpdate: key, compType: compType } })
    }

    function viewUpdateDelete(data, key) {

        let paymentTerms_id = data.payment_terms_id
        setPaymenttermsId(paymentTerms_id)
        switch (key) {
            case 'update': infoForUpdate(paymentTerms_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(paymentTerms_id, 'view'); break;
        }

    }

    async function DeletePaymentTerms() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/paymentterms/FnDeleteRecord/${paymenttermsId}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("PaymentTerms Deleted: ", responce);
            setShow(false)
    ...
    //add fuction call
    const openForm = () => {
        navigate("/Masters/FrmPaymentTermsEntry", { state: { paymentTermsID: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['payment_terms_id']
            ...
            jsonQuery['viewname']['cmv_payment_terms'] = 'cmv_payment_terms';
            jsonQuery['orderBy']['ORDER BY'] = 'payment_terms_id';
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Payment Terms</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeletePaymentTerms} />
...
export default FrmPaymentTemrsListing;