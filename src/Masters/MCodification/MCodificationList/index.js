...
function MCodificationListing({ compType = 'Masters' }) {
    const configConstants = ConfigConstants();
    const { COMPANY_ID, COMPANY_NAME, USER_ACCESS, UserName } = configConstants;
    ...
    const [product_codification_id, setProductCodificationId] = useState(0)
    let reportName = "Codification Report";
    ...
    // Table Data
    const [data, setCodificationData] = useState([]);
    ...
    async function deleteCodification() {
        debugger
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/SmProductCodification/FnDeleteRecord/${product_codification_id}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("Codification Deleted: ", responce);
            setShow(false)
    ...
    const FnShowAllReportRecords = async (reportIdentifierKey) => {
        try {
            debugger
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=reportType=${'summary'}&viewName=${'smv_product_codification_rpt'}`)
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                                ...
                                <div style={{ display: "flex" }}>
                                    <Tooltip title="view" placement="top">
                                        <MDTypography className="erp-view-btn" >
                                            <AiFillEye className={`erp-view-btn ${access_control.all_access || access_control.read_access ? 'display' : 'd-none'}`} onClick={e => viewUpdateDelete(row.original, 'view')} />
                                        </MDTypography>
                                    </Tooltip>
                                    ...
                                    {compType === 'Register' ?
                                        null : <>
                                            ...
                                            <Tooltip title='Delete' placement="top">
                                                <MDTypography className="erp-view-btn">
                                                    <MdDelete className={`erp-delete-btn ${access_control.all_access || access_control.delete_access ? 'display' : 'd-none'}`} onClick={e => viewUpdateDelete(row.original, 'delete')} />
                                                </MDTypography>
                                            </Tooltip>
                                        </>
                                    }
                ...
                setColumns(nColumn);
                setCodificationData(responce.content);
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['product_codification_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'product_codification_id';

            reportName = "Codification Report"
            jsonQuery['viewname']['viewName'] = 'smv_product_codification'
    ...
    function viewUpdateDelete(data, key) {
        setProductCodificationId(data.product_codification_id)
        switch (key) {
           
            case 'update': infoForUpdate(data, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(data, 'view'); break;
        }
    }
    ...
    const infoForUpdate = (data, key) => {
        navigate("/Masters/MCodification/FrmMCodificationEntry", { state: { product_codification_id: data.product_codification_id, keyForViewUpdate: key, compType: compType } })
    }
    ...
    const openForm = () => {
        navigate("/Masters/MCodification/FrmMCodificationEntry", { state: { idList: '', keyForViewUpdate: '', compType: compType } })
    }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Codification</label>
                </div>
                ...
                {data.length !== 0 ? <>
                    ...
                    <Datatable data={data} columns={columns} freezeColumnCount={3} stateValue={searchInputValue} />
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={deleteCodification} />
        </>
    )
}

export default MCodificationListing;
