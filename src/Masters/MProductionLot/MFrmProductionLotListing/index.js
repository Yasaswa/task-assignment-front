...
function MFrmProductionLotListing({ compType = 'Masters' }) {
    ...
    // Table Data
    const [data, setLotData] = useState([]);
    ...
    const csvExp = useRef();
    const pdfExpFilters = {};
    var reportName = "Production Lot Report"
    ...
    useEffect(() => {
        const functionCall = async () => {
            ...
            const availCols = await FnShowProdutionlotRptRecords(reportType);
            await fetchFilteredData(PageCurrent, entriesPerPage, availCols)
        }
        functionCall()
    }, [compType])

    const FnShowProdutionlotRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'xmv_production_lot_master_rpt'}`)
    ...
    const infoForUpdate = (lot_id, key) => {
        navigate("/Masters/MProductionLot/MFrmProductionLotEntry", { state: { lotid: lot_id, keyForViewUpdate: key, compType: compType } })
    }

    function viewUpdateDelete(data, key) {
        var lot_id = data.lot_id
        setlotId(lot_id)
        switch (key) {
            case 'update': infoForUpdate(lot_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(lot_id, 'view'); break;
        }

    }

    async function Deletelotdata() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/XmProductionLotMaster/FnDeleteRecord/${lotid}/${COMPANY_ID}/${UserName}`, method)
    ...
    const openForm = () => {
        navigate("/Masters/MProductionLot/MFrmProductionLotEntry", { state: { lotid: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'lot_id'
            jsonQuery['viewname']['hmv_job_type'] = 'xmv_production_lot_master';
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Production Lot</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={Deletelotdata} />
        </>
    )
}
export default MFrmProductionLotListing;