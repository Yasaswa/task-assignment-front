...
function FrmTaskMasterListing() {
    var storeSelectedValues = [];
    const [task_transaction_id, setTransactionId] = useState([]);
    const [selectedFilters, setSelectedFilters] = useState([]);
    //table Feilds
    const configConstants = ConfigConstants();
    const { COMPANY_ID, UserName, COMPANY_NAME, USER_ACCESS } = configConstants;
    const [data, setDepartmentData] = useState([]);
    ...
    const pdfExpFilters = {};
    var reportName = "Department Report"
    ...
    useEffect(() => {
        const loadDataOnLoad = async () => {
            ...
            $(".erp_group-reslut-by").hide();
            const availCols = await FnShowDepartmentRptRecords();
    ...
    const FnShowDepartmentRptRecords = async () => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'mtv_task_assign_master_rpt'}`)
    ...
    const infoForUpdate = async (task_transaction_id, key) => {
        navigate("/Masters/MTaskMaster/MTaskMasterEntry", { state: { taskTransactionId: task_transaction_id, keyForViewUpdate: key } })
    }

    function viewUpdateDelete(data, key) {
        var task_transaction_id = data.task_transaction_id
        setTransactionId(task_transaction_id)
        switch (key) {
            case 'update': infoForUpdate(task_transaction_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(task_transaction_id, 'view'); break;
        }
    }
    ...
    async function DeleteRecord() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/MTaskAssign/FnDeleteRecord/${task_transaction_id}/${UserName}/${COMPANY_ID}`, method)
    ...
    const openForm = () => {
        navigate("/Masters/MTaskMaster/MTaskMasterEntry", { state: { taskTransactionId: 0, keyForViewUpdate: 'Add' } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['task_transaction_id']
            ...
            jsonQuery['viewname']['mtv_task_assign_master'] = 'mtv_task_assign_master';
            jsonQuery['orderBy']['ORDER BY'] = 'task_transaction_id';
            ...
            if (responce.content.length > 0) {
                ...
                for (let colKey = 0; colKey < availCols.length; colKey++) {
                    ...
                    nColumn.push({
                        ...
                        width:
                            colNameAccessor === 'employee_name' || colNameAccessor === 'task_name' || colNameAccessor === 'employee_designation'
                                ? 200
                                : colNameAccessor === 'task_purpose'
                                    ? 350
                                    : undefined
                    });
                ...
                setColumns(nColumn);
                setDepartmentData(responce.content)
            } else {
                setColumns([]);
                setDepartmentData([])
            }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Task Assign Listing</label>
                </div>
                ...
                <div className="row btn_row_class">
                    <div className="col-sm-8" >
                        <MDButton className={`btn erp-gb-button`} variant="button" fontWeight="regular" onClick={openForm}>Add</MDButton> &nbsp;
...
export default FrmTaskMasterListing