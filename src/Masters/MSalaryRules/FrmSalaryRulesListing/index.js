...
function FrmSalaryRulesListing({ compType = 'Masters' }) {
    ...
    let reportName = "Salary Rules Report";

    // Popup Fields
    const [show, setShow] = useState(false);
    const handleClose = () => setShow(false);

    const [salary_rule_id, setSalaryRuleId] = useState();
    ...
    const FnShowAllReportRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'hmv_salary_rules_rpt'}`)
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['salary_rule_id', 'property_id']
            ...
            jsonQuery['viewname']['viewName'] = 'hmv_salary_rules';
            jsonQuery['orderBy']['ORDER BY'] = 'salary_rule_id';
    ...
    const openForm = () => {
        navigate("/Masters/MSalaryRules/FrmSalaryRulesEntry", { state: { salaryRuleId: 0, keyForViewUpdate: 'Add', compType: compType } })
    }


    const infoForUpdate = async (salary_rule_id, property_id, key) => {
        navigate("/Masters/MSalaryRules/FrmSalaryRulesEntry", { state: { salaryRuleId: salary_rule_id, propertyId : property_id, keyForViewUpdate: key, compType: compType } })
    }


    function viewUpdateDelete(data, key) {
        var salary_rule_id = data.salary_rule_id
        var property_id = data.property_id;
        setSalaryRuleId(salary_rule_id)
        switch (key) {
            case 'update': infoForUpdate(salary_rule_id, property_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(salary_rule_id, property_id, 'view'); break;
        }
    }
    ...
    async function DeleteRecord() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/salaryRules/FnDeleteRecord/${salary_rule_id}/${COMPANY_ID}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("Salary Rules Deleted: ", responce);
            setShow(false)
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Salary Rules</label>
                </div>
...
export default FrmSalaryRulesListing