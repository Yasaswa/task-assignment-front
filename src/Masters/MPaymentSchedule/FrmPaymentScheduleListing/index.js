...
function FrmPaymentScheduleListing({ compType = 'Masters' }) {
    ...
    // Table Data
    const [data, setPaymentScheduleData] = useState([]);
    ...
    const pdfExpFilters = {};
    var reportName = "PaymentSchedule Report"
  ...
  var operatorByType = {
    ...
    useEffect(async () => {
        ...
        const availCols = await FnShowPaymentScheduleRptRecords(reportType);
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setPaymentScheduleData(responce.content)
            } else {
                setColumns([]);
    ...
    const FnShowPaymentScheduleRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_payment_schedule_rpt'}`)
    ...
    const infoForUpdate = async (payment_schedule_id, key) => {
        navigate("/Masters/FrmPaymentScheduleEntry", { state: { paymentScheduleID: payment_schedule_id, keyForViewUpdate: key, compType: compType } })
    }

    function viewUpdateDelete(data, key) {

        let paymentSchedule_id = data.payment_schedule_id
        setPaymentscheduleId(paymentSchedule_id)
        switch (key) {
            case 'update': infoForUpdate(paymentSchedule_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(paymentSchedule_id, 'view'); break;
        }

    }

    async function DeletePaymentSchedule() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/CmPaymentSchedule/FnDeleteRecord/${paymentScheduleId}/${COMPANY_ID}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("PaymentSchedule Deleted: ", responce);
            setShow(false)
    ...
    //add fuction call
    const openForm = () => {
        navigate("/Masters/FrmPaymentScheduleEntry", { state: { paymentScheduleID: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['payment_schedule_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'payment_schedule_id';
            jsonQuery['viewname']['cmv_payment_schedule'] = 'cmv_payment_schedule';
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Payment Schedule</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeletePaymentSchedule} />
        </>
    );
}

export default FrmPaymentScheduleListing;