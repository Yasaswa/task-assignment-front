...
function FrmStandardLossListing({ compType = 'Masters' }) {

  var storeSelectedValues = [];
  //session ID

  const [prod_standard_losses_id, setStdProdLossId] = useState();
  ...
  var reportName = "Standard Loss Report"
  ...
  useEffect(async () => {
    ...
    const availCols = await FnShowCountRptRecords(reportType);
  ...
  const FnShowCountRptRecords = async (reportIdentifierKey) => {
    try {
      const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'xt_production_standard_losses_rpt'}`)
  ...
  const infoForUpdate = async (production_standard_losses_id, key) => {

    navigate("/Masters/MStandardLoss/StandardLossEntry", { state: { standardLossId: production_standard_losses_id, keyForViewUpdate: key, compType: compType } })

  }

  function viewUpdateDelete(data, key) {
    debugger
    var standard_losses_id = data.production_standard_losses_id
    setStdProdLossId(standard_losses_id)
    switch (key) {
      case 'update': infoForUpdate(standard_losses_id, 'update'); break;
      case 'delete': setShow(true); break;
      case 'view': infoForUpdate(standard_losses_id, 'view'); break;
    }
  }

  const openForm = () => {
    navigate("/Masters/MStandardLoss/StandardLossEntry", { state: { standardLossId: 0, keyForViewUpdate: 'Add', compType: compType } })
  }
  ...
  const submitQuery = async (availCols) => {
    try {
      const requiredColumns = ['production_standard_losses_id']
      ...
      jsonQuery['viewname']['xt_production_standard_losses'] = 'xt_production_standard_losses';
      jsonQuery['orderBy']['ORDER BY'] = 'production_standard_losses_id';
  ...
  async function Deletecount() {
    ...
    try {
      ...
      const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/XtProductionStandardLoss/FnDeleteRecord/${parseInt(prod_standard_losses_id)}/${parseInt(COMPANY_ID)}/${UserName}`, method);
    ...
    <>
      ...
      <DashboardLayout>
        <div className='main_heding'>
          <label className='erp-form-label-lg'>Standard Loss </label>
        </div>
      ...
      {/* Delete Modal */}
      <DeleteModal show={show} closeModal={handleClose} deleteRecord={Deletecount} />
    </>
  )
}

export default FrmStandardLossListing      
