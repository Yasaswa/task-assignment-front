...
function FrmCommunicationsMasterList({ compType = 'Masters' }) {
  ...
  const [communications_templatesID, Setcommunications_templatesID] = useState();


  //table Feilds
  const [data, setCommunicationsData] = useState([]);
  ...
  var reportName = "Template Communications Report"
  ...
  useEffect(async () => {
    ...
    const availCols = await showCommunicationsReportRecords(reportType);
    await fetchFilteredData(PageCurrent, entriesPerPage, availCols)

  }, [compType])


  const showCommunicationsReportRecords = async (reportIdentifierKey) => {
    debugger
    try {
      const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'amv_templates_communications_rpt'}`)
      const summaryResponce = await summaryRptApiCall.json();
      console.log("summaryResponce responce: ", summaryResponce)
      if (Object.keys(summaryResponce.content).length !== 0) {
  ...
  const infoForUpdate = async (communications_templates_id, key) => {

    navigate("/Masters/MCommunicationsMaster/FrmCommunicationsMasterEntry", { state: { communications_templatesID: communications_templates_id, keyForViewUpdate: key, compType: compType } })
  }

  function viewUpdateDelete(data, key) {
    var communications_templates_id = data.communications_templates_id
    Setcommunications_templatesID(communications_templates_id)
    switch (key) {
      case 'update': infoForUpdate(communications_templates_id, 'update'); break;
      case 'delete': setShow(true); break;
      case 'view': infoForUpdate(communications_templates_id, 'view'); break;
    }
  }

  const openForm = () => {
    navigate("/Masters/MCommunicationsMaster/FrmCommunicationsMasterEntry", { state: { communications_templatesID: 0, keyForViewUpdate: 'Add', compType: compType } })
  }
  ...
  const submitQuery = async (availCols) => {
    try {
      const requiredColumns = ['communications_templates_id']
      ...
      jsonQuery['orderBy']['ORDER BY'] = "communications_templates_id";
      jsonQuery['viewname']['viewName'] = 'amv_templates_communications'
  ...
  const fetchFilteredData = async (page, size, availCols) => {
    ...
    try {
      ...
      if (responce.content.length > 0) {
        ...
        setColumns(nColumn);
        setCommunicationsData(responce.content)
      } else {
        setColumns([]);
  ...
  function DeleteCommunications() {
    try {
      fetch(`${process.env.REACT_APP_BASE_URL}/api/AmTemplatesCommunications/FnDeleteRecord/${communications_templatesID}/${COMPANY_ID}/${UserName}`, {

        method: 'Delete'
      }).then((result) => {
        result.json().then((responce) => {
          console.log(" Communications  Deleted: ", responce);
          setShow(false)
    ...
    <>
      ...
      <DashboardLayout>
        <div className='main_heding'>
          <label className='erp-form-label-lg'>Templates Communications</label>
        </div>
      ...
      {/* Delete Modal */}
      <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteCommunications} />
    </>

  )
}

export default FrmCommunicationsMasterList
