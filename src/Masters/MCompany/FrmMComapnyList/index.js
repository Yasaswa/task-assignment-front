...
function FrmMCompanyList({ compType = 'Masters' }) {
    ...
    useEffect(async () => {
        ...
        const availCols = await FnShowCompanyRptRecords(reportType);
    ...
    const FnShowCompanyRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_company_rpt'}`)
    ...
    const infoForUpdate = async (company_id, key) => {
        sessionStorage.removeItem('keyForViewUpdate');
        sessionStorage.removeItem('compType');
        ...
        sessionStorage.setItem('compType', compType);
    ...
    function viewUpdateDelete(data, key) {
        ...
        switch (key) {
            case 'update': infoForUpdate(company_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(company_id, 'view'); break;
        }
    ...
    async function DeleteComapny() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/company/FnDeleteRecord/${companyId}`, method)
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setCompanyData(responce.content)
            } else {
                setColumns([]);
                setCompanyData([])
            }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['company_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = "company_id";
            jsonQuery['viewname']['viewName'] = 'cmv_company'
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Company Information</label>
                </div>
            ...
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteComapny} />


        </>
    )
}
export default FrmMCompanyList;