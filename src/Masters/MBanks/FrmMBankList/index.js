...
function FrmMBankList({ compType = 'Masters' }) {
    ...
    useEffect(() => {
        const functionCall = async () => {
            ...
            const availCols = await FnShowBankRptRecords(reportType);
    ...
    const FnShowBankRptRecords = async (reportIdentifierKey) => {
        try {
            debugger;
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_banks_rpt'}`)
    ...
    const infoForUpdate = (bank_id, key) => {
        navigate("/Masters/MBanks/FrmBank/FrmMBankEntry", { state: { bankID: bank_id, keyForViewUpdate: key, compType: compType } })
    }
    ...
    const openForm = () => {
        navigate("/Masters/MBanks/FrmBank/FrmMBankEntry", { state: { bankID: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    function DeleteBank() {
        try {
            fetch(`${process.env.REACT_APP_BASE_URL}/api/bank/FnDeleteRecord/${BankId}/${COMPANY_ID}/${UserName}`, {
                method: 'Delete'
            }).then((result) => {
                result.json().then((responce) => {
                    console.log(" Bank  Deleted: ", responce);
                    setShow(false)
    ...
    const submitQuery = async (availCols) => {
        try {
            ...
            jsonQuery['viewname']['cmv_banks'] = 'cmv_banks';
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Bank</label>
                </div>
            ...
            {/* Delete Modal */}
            < DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteBank} />
        </>
