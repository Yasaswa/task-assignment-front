...
function FrmCompOffLeaveList({ compType = 'Transactions' }) {
    ...
    let reportName = "CompOff Leave Request Report";
    ...
    const FnShowAllReportRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'htv_comp_off_intimation_details_rpt'}`)
                                ...
                                <div style={{ display: "flex" }}>
                                    ...
                                    {compType === 'Register' ?
                                        null : <>
                                            ...
                                            <Tooltip title={row.original.status === 'Approved' ? 'Comp Off Approved' : row.original.status === 'Rejected' ? 'Comp Off Rejected' : row.original.status === 'Cancelled' ? 'Comp Off Cancel' : 'Approved'} placement="top">
                                                {/* <MDTypography className={`erp-view-btn ${employeeDetailsApiCall[0].department_name === 'Human Resource Management' || row.original.reporting_to_name  === employeeDetailsApiCall[0].field_name ? 'display' : 'd-none'}`} > */}
                                                {/* <MDTypography className={`erp-view-btn ${employeeDetailsApiCall[0] &&
                                                    (employeeDetailsApiCall[0].department_name === 'Human Resource Management' || row.original.reporting_to === employeeDetailsApiCall[0].field_id) ?
                                                    'display' :
                                                    'd-none'
                                                    }`}> */}
                                                <MDTypography
                                                    className={`erp-view-btn ${row.original.reporting_to === parseInt(UserId) ? 'display' : 'd-none'}`}>

                                                    <BsFillCheckCircleFill className="erp-view-btn" onClick={e => {
                                                        if (row.original.status !== 'Approved' && row.original.status !== 'Rejected' && row.original.status !== 'Cancelled') {
                                                            viewUpdateDelete(row.original, 'approve', access_control)
                                                        }
                                                    }} disabled={row.original.status === 'Approved' || row.original.status === 'Rejected' || row.original.status === 'Cancelled'} />
                                                </MDTypography>
                                            </Tooltip>

                                            {/* <Tooltip title={row.original.status === 'Approved' ? 'Comp Off Approved' : row.original.status === 'Rejected' ? 'Comp Off Rejected' : row.original.status === 'Cancelled' ? 'Comp Off Cancel' : 'Edit'} placement="top">
                                                <MDTypography className="erp-view-btn" >
                                                    <MdModeEdit className={`erp-edit-btn ${access_control.all_access || access_control.read_access ? 'display' : 'd-none'}`} onClick={e => {
                                                        if (row.original.status !== 'Approved' && row.original.status !== 'Rejected' && row.original.status !== 'Cancelled') {
                                            ...
                                            {row.original.created_by === UserName && (
                                                <Tooltip title={row.original.status === 'Approved' ? 'Comp Off Approved' : row.original.status === 'Rejected' ? 'Comp Off Rejected' : row.original.status === 'Cancelled' ? 'Comp Off Cancel' : 'Delete'} placement="top">
                                                    <MDTypography className="erp-view-btn" >
                                                        <MdDelete className={`erp-delete-btn ${access_control.all_access || access_control.read_access ? 'display' : 'd-none'}`} onClick={e => {
                                                            if (row.original.status !== 'Approved' && row.original.status !== 'Rejected' && row.original.status !== 'Cancelled') {
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['comp_off_intimation_details_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'comp_off_intimation_details_id'
            jsonQuery['viewname']['htv_comp_off_intimation_details'] = 'htv_comp_off_intimation_details';
            if (UserId !== "1" || UserId !== 1 || UserId !== "3" || UserId !== 1) {
    ...
    function viewUpdateDelete(data, key, access_control) {
        ...
        var comp_off_intimation_details_id = data.comp_off_intimation_details_id
        setCompoffId(comp_off_intimation_details_id)
        setPunchCode(data.punch_code)
        setDateTime(data.att_date_time)


        if (key === 'view' || key === "update") {
            infoForUpdate(comp_off_intimation_details_id, key, modules_forms_id);
        }
        if (data.status !== 'Approved' && data.status !== 'Rejected' && data.status !== 'Cancelled') {
            switch (key) {
                // case 'update': infoForUpdate(comp_off_intimation_details_id, 'update', modules_forms_id); break;
                case 'approve': infoForUpdate(comp_off_intimation_details_id, 'approve', modules_forms_id); break;
                case 'delete': setShow(true); break;
                case 'cancel': infoForUpdate(comp_off_intimation_details_id, 'cancel', modules_forms_id); break;
                case 'add': infoForUpdate(comp_off_intimation_details_id, 'add', modules_forms_id); break;
                default: break;
            }
        }
    }
    ...
    const deleteRecords = async () => {
        ...
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/HmCompoffDetails/FnDeleteRecord/${punch_code}/${COMPANY_ID}/${datetime}/${UserName}`, method)
    ...
    const infoForUpdate = async (comp_off_intimation_details_id, key, modules_forms_id) => {
        if (key === 'view') {
            navigate("/Masters/MCompOffLeaveRequest/FrmCompOffLeaveEntry", { state: { compoffId: comp_off_intimation_details_id, keyForViewUpdate: 'view', compType: compType, modules_forms_id: modules_forms_id } })
        } else {
            navigate("/Masters/MCompOffLeaveRequest/FrmCompOffLeaveEntry", { state: { compoffId: comp_off_intimation_details_id, keyForViewUpdate: key, compType: compType, modules_forms_id: modules_forms_id } })
        }
    }
    ...
    const openForm = () => {
        ...
        navigate("/Masters/MCompOffLeaveRequest/FrmCompOffLeaveEntry", { state: { compoffId: 0, keyForViewUpdate: 'add', compType: compType, modules_forms_id: access_control.modules_forms_id } })
    }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Comp.Off & Holiday Present Leave</label>
                </div>
...
export default FrmCompOffLeaveList;
