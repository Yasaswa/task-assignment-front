...
function FrmHolidayListing({ compType = 'Masters' }) {
    ...
    const [production_holiday_id, setPropertyId] = useState();


    //table Feilds

    const [data, setHolidayData] = useState([]);
    ...
    var reportName = "production Holiday Report"
    ...
    useEffect(async () => {
        ...
        await FnShowHolidayRptRecords();
        const availCols = await FnShowHolidayRptRecords(reportType);
        await fetchFilteredData(PageCurrent, entriesPerPage, availCols)

    }, [compType])

    const FnShowHolidayRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'xmv_production_holiday_rpt'}`)
    ...
    const infoForUpdate = async (production_holiday_id, key) => {

        navigate("/Masters/MHoliday/FrmHolidayEntry", { state: { productionHolidayId: production_holiday_id, keyForViewUpdate: key, compType: compType } })

    }

    function viewUpdateDelete(data, key) {
        var production_holiday_id = data.production_holiday_id
        setPropertyId(production_holiday_id)
        switch (key) {
            case 'update': infoForUpdate(production_holiday_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(production_holiday_id, 'view'); break;
        }
    }

    const openForm = () => {
        navigate("/Masters/MHoliday/FrmHolidayEntry", { state: { productionHolidayId: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['production_holiday_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'production_holiday_id';
            jsonQuery['viewname']['amv_properties'] = 'xmv_production_holiday';
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setHolidayData(responce.content)
            } else {
                setColumns([]);
                setHolidayData([])
            }
    ...
    async function DeleteHolidayRecord() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/XmProductionHoliday/FnDeleteRecord/${production_holiday_id}/${UserName}/${COMPANY_ID}`, method)
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Production Holiday</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteHolidayRecord} />
        </>
    )
}


export default FrmHolidayListing      
