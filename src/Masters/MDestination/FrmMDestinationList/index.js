...
function FrmMDestinationList({ compType = 'Masters' }) {
    ...
    useEffect(async () => {
        ...
        const availCols = await FnShowDestinationRptRecords(reportType);
        await fetchFilteredData(PageCurrent, entriesPerPage, availCols)
    }, [compType])


    const FnShowDestinationRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_destination_rpt'}`)
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setDestinationData(responce.content)
            } else {
                setColumns([]);
                setDestinationData([])
            }
    ...
    const openForm = () => {
        navigate("/Masters/Destination", { state: { destinationID: 0, keyForViewUpdate: 'Add', compType: compType } })
    }

    const infoForUpdate = async (destination_id, key) => {
        navigate("/Masters/Destination", { state: { destinationID: destination_id, keyForViewUpdate: key, compType: compType } })
    }
    ...
    function viewUpdateDelete(data, key) {
        ...
        switch (key) {
            case 'update': infoForUpdate(destination_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(destination_id, 'view'); break;
        }
    ...
    async function DeleteDestination() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/destination/FnDeleteRecord/${destinationId}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("Destination Deleted: ", responce);
            setShow(false)
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['destination_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'destination_id';
            jsonQuery['viewname']['cmv_destination'] = 'cmv_destination';
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Destination Information</label>
                </div>
            ...
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteDestination} />
        </>
    )
}

export default FrmMDestinationList
