...
function FrmShiftListing({ compType = 'Masters' }) {
    ...
    const FnShowAllReportRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_shift_rpt'}`)
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['shift_id']
            ...
            jsonQuery['viewname']['viewName'] = 'cmv_shift';
            jsonQuery['orderBy']['ORDER BY'] = 'shift_id';
    ...
    const openForm = () => {
        navigate("/Masters/FrmShiftEntry", { state: { shiftId: 0, keyForViewUpdate: '', compType: compType } })
    }


    const infoForUpdate = async (shift_id, key) => {
        navigate("/Masters/FrmShiftEntry", { state: { shiftId: shift_id, keyForViewUpdate: key, compType: compType } })
    }
    ...
    function viewUpdateDelete(data, key) {
        ...
        setShiftId(shift_id)
        switch (key) {
            case 'update': infoForUpdate(shift_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(shift_id, 'view'); break;
        }
    }
    ...
    async function DeleteRecord() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/shift/FnDeleteRecord/${shift_id}/${COMPANY_ID}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("Shift Deleted: ", responce);
            setShow(false)
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Shift</label>
                </div>
...
export default FrmShiftListing
