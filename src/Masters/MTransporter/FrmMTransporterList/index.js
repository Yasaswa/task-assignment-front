...
function FrmMTransporterList({ compType = 'Masters' }) {
    // Call ConfigConstants to get the configuration constants
    const configConstants = ConfigConstants();
    const { COMPANY_ID, USER_ACCESS, UserName, COMPANY_NAME} = configConstants;

    var storeSelectedValues = [];
    const [TransportarId, setTransportarId] = useState('');
    ...
    //Table Data
    const [data, setTransporterData] = useState([]);
    ...
    const csvExp = useRef();
    const pdfExpFilters = {};
    var reportName = "Transporter Report";
    ...
    useEffect(async () => {
        ...
        // FnShowTransporterRptRecords();
        const availCols = await FnShowTransporterRptRecords(reportType);
        await fetchFilteredData(PageCurrent, entriesPerPage, availCols)

    }, [compType, reportType])

    const FnShowTransporterRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_transporter_rpt'}`)
    ...
    const openForm = () => {
        navigate("/Masters/Transporter", { state: { transporterId: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    function viewUpdateDelete(data, key) {
        ...
        setTransportarId(transporter_id)
        switch (key) {
    ...
    const infoForUpdate = ({ transporter_id, transporter_name, transporter_vendor_code }, key) => {
        debugger
        navigate("/Masters/Transporter", { state: { transporterId: transporter_id, transporter_name: transporter_name, transporter_vendor_code: transporter_vendor_code, keyForViewUpdate: key, compType: compType } })
    }

    function DeleteTransporter() {
        fetch(`${process.env.REACT_APP_BASE_URL}/api/transporter/FnDeleteRecord/${TransportarId}/${UserName}`, {
            method: 'Delete'
        }).then((result) => {
            result.json().then((responce) => {
                console.log("Transporter  Deleted: ", responce);
                setShow(false)
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['transporter_id']
            ...
            jsonQuery['viewname']['viewName'] = 'cmv_transporter'
            jsonQuery['orderBy']['ORDER BY'] = 'transporter_id';
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setTransporterData(responce.content)
            } else {
                setColumns([]);
                setTransporterData([])
            }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Transporter Information</label>
                </div>
            ...
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteTransporter} />

        </>
    )
}
export default FrmMTransporterList;