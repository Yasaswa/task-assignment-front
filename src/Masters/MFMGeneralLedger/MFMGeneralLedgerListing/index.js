...
function MFMGeneralLedgerListing({ compType = 'Masters' }) {
    const [general_ledger_id, setgeneral_ledger_id] = useState();
    ...
    // Table Data
    const [data, setGeneralLedgerData] = useState([]);
    ...
    const pdfExpFilters = {};
    var reportName = "GeneralLedger Report"
    ...
    useEffect(async () => {
        ...
        const availCols = await FnShowGeneralledgerRptRecords(reportType);
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setGeneralLedgerData(responce.content)
            } else {
                setColumns([]);
    ...
    const FnShowGeneralledgerRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'fmv_general_ledger_rpt'}`)
    ...
    const infoForUpdate = (general_ledger_id, key) => {
        navigate("/Masters/MFMGeneralLedgerEntry", { state: { generalledgerId: general_ledger_id, keyForViewUpdate: key, compType: compType } })
    }

    function viewUpdateDelete(data, key) {
        var general_ledger_id = data.general_ledger_id
        setgeneral_ledger_id(general_ledger_id)
        switch (key) {
            case 'update': infoForUpdate(general_ledger_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(general_ledger_id, 'view'); break;
        }

    }

    async function DeleteGeneralLeader() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/FmGeneralLedger/FnDeleteRecord/${general_ledger_id}/${COMPANY_ID}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("GeneralLeader Deleted: ", responce);
            setShow(false)
    ...
    const openForm = () => {
        navigate("/Masters/MFMGeneralLedgerEntry", { state: { generalledgerId: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['general_ledger_id']
            ...
            jsonQuery['viewname']['fmv_general_ledger'] = 'fmv_general_ledger';
            jsonQuery['orderBy']['ORDER BY'] = 'general_ledger_id';
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>General Ledger</label>
                </div>
            ...
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteGeneralLeader} />
        </>
    )
...
export default MFMGeneralLedgerListing