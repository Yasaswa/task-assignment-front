...
function FrmMHMEarningHeadsListing({ compType = 'Masters' }) {
    ...
    //session ID
    const [earning_headsID, setearning_headsID] = useState();


    //table Feilds
    const [data, setEarningHeadsData] = useState([]);
    ...
    var reportName = "Earning Heads Report"
    ...
    useEffect(async () => {
        ...
        const availCols = await showEarningHeadsIDReportRecords(reportType);
    ...
    const showEarningHeadsIDReportRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'hmv_earning_heads_rpt'}`)
    ...
    const infoForUpdate = async (earning_headsID, key) => {
        navigate("/Masters/MHMEarningHeads/FrmMHMEarningHeadsEntry", { state: { earning_headsIDs: earning_headsID, keyForViewUpdate: key, compType: compType } })
    }


    function viewUpdateDelete(data, key) {

        var earningHeadsId = data.earning_heads_id
        setearning_headsID(earningHeadsId)
        switch (key) {

            case 'update': infoForUpdate(earningHeadsId, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(earningHeadsId, 'view'); break;

        }
    }

    const openForm = () => {
        navigate("/Masters/MHMEarningHeads/FrmMHMEarningHeadsEntry", { state: { earning_headsIDs: 0, keyForViewUpdate: '', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['earning_heads_id', 'earning_head_version']
            ...
            jsonQuery['orderBy']['ORDER BY'] = "earning_heads_id";
            jsonQuery['viewname']['viewName'] = 'hmv_earning_heads'
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setEarningHeadsData(responce.content)
            } else {
                setColumns([]);
                setEarningHeadsData([])
            }
    ...
    function DeleteEarningHeads() {
        try {
            fetch(`${process.env.REACT_APP_BASE_URL}/api/HmEarningHeads/FnDeleteRecord/${earning_headsID}/${COMPANY_ID}/${UserName}`, {

                method: 'Delete'
            }).then((result) => {
                result.json().then((responce) => {
                    console.log(" EarningHeadsData  Deleted: ", responce);
                    setShow(false)
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Earning Heads</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteEarningHeads} />
        </>

    )
}

export default FrmMHMEarningHeadsListing          
