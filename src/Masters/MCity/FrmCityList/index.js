...
export default function FrmCityList({ compType = 'Masters' }) {
  ...
  const FnShowCityRptRecords = async (reportIdentifierKey) => {
    try {
      const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_city_rpt'}`)
  ...
  const infoForUpdate = async (city_id, key) => {
    navigate("/Masters/City", { state: { CityId: city_id, keyForViewUpdate: key, compType: compType } })
  }
  //end chgenges
  ...
  function viewUpdateDelete(data, key) {
    ...
    setCityID(city_id)
    switch (key) {
      case 'update': infoForUpdate(city_id, 'update'); break;
      case 'delete': setShow(true); break;
      case 'view': infoForUpdate(city_id, 'view'); break;
    }
  }

  const openForm = () => {
    navigate("/Masters/City", { state: { CityId: 0, keyForViewUpdate: 'Add', compType: compType } })
  }

  async function DeleteCity() {
    try {
      const method = { method: 'DELETE' }
      const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/city/FnDeleteRecord/${cityId}/${UserName}`, method)
  ...
  const submitQuery = async (availCols) => {
    try {
      ...
      jsonQuery['orderBy']['ORDER BY'] = 'city_id';
      jsonQuery['viewname']['cmv_city'] = 'cmv_city';
    ...
    <>
      ...
      <DashboardLayout>
        <div className='main_heding'>
          <label className='erp-form-label-lg'>City Information</label>
        </div>
