...
function ProfitCenterListing({ compType = 'Masters' }) {
    ...
    // Table Data
    const [data, setprofitcenterData] = useState([]);
    ...
    const pdfExpFilters = {};
    var reportName = "Profit Center Report"
    ...
    useEffect(() => {
        const functionCall = async () => {
            ...
            const availCols = await FnShowProfitcenterRptRecords(reportType);
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setprofitcenterData(responce.content)
            } else {
                setColumns([]);
                setprofitcenterData([])
            }
    ...
    const FnShowProfitcenterRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'fmv_profit_center_rpt'}`)
    ...
    const infoForUpdate = async (profit_center_id, key) => {

        navigate("/Masters/ProfitCenterEntry", { state: { profitcenterID: profit_center_id, keyForViewUpdate: key, compType: compType } })
    }
    function viewUpdateDelete(data, key) {
        let profit_center_id = data.profit_center_id
        setprofitcenterID(profit_center_id)
        switch (key) {
            case 'update': infoForUpdate(profit_center_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(profit_center_id, 'view'); break;
        }

    }

    async function DeleteProfitCenter() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/FmProfitCenter/FnDeleteRecord/${profitcenterID}/${COMPANY_ID}/${UserName}`, method)
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['profit_center_id']
            ...
            jsonQuery['viewname']['fmv_profit_center'] = 'fmv_profit_center';
            jsonQuery['orderBy']['ORDER BY'] = 'profit_center_id';
    ...
    const openForm = () => {
        navigate("/Masters/ProfitCenterEntry", { state: { profitcenterID: 0, keyForViewUpdate: '', compType: compType } })
    }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Profit Center</label>
                </div>
            ...
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteProfitCenter} />
        </>
    )
}

export default ProfitCenterListing
