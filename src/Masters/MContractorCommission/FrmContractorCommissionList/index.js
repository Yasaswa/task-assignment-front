...
function FrmContractorCommissionList({ compType = 'Masters' }) {
    ...
    // Table Data
    const [data, setContractCommissionData] = useState([]);
    ...
    let [optionBox, setOptionBox] = useState([]);
    ...
    const csvExp = useRef();
    const pdfExpFilters = {};
    var reportName = "Contractor Commission Report"
    ...
    useEffect(() => {
        const functionCall = async () => {
            ...
            const availCols = await FnShowContractCommissionRptRecords(reportType);
            await fetchFilteredData(PageCurrent, entriesPerPage, availCols)
        }
        functionCall()
    }, [compType])

    const FnShowContractCommissionRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'hmv_commission_rate_rpt'}`)
    ...
    const infoForUpdate = (commission_rate_id, key) => {
        navigate("/Masters/MContractorCommission/FrmContractorCommissionEntry", { state: { commissionId: commission_rate_id, keyForViewUpdate: key, compType: compType } })
    }

    function viewUpdateDelete(data, key) {
        var commission_rate_id = data.commission_rate_id
        setCommissionId(commission_rate_id)
        switch (key) {
            case 'update': infoForUpdate(commission_rate_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(commission_rate_id, 'view'); break;
        }

    }

    async function DeleteContractCommission() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/contractCommission/FnDeleteRecord/${commissionId}/${UserName}`, method)
    ...
    const openForm = () => {
        navigate("/Masters/MContractorCommission/FrmContractorCommissionEntry", { state: { commissionId: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['commission_rate_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'commission_rate_id'
            jsonQuery['viewname']['hmv_commission_rate'] = 'hmv_commission_rate';
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setContractCommissionData(responce.content)
            } else {
                setColumns([]);
                setContractCommissionData([])
            }
            return responce;
        } catch (error) {
            console.log("error: ", error)
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Contractor Commission</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteContractCommission} />
        </>
    )
}
export default FrmContractorCommissionList;