...
function FrmCostCenterHeadsListing({ compType = 'Masters' }) {
    ...
    // Table Data
    const [data, setcostcenterData] = useState([]);
    ...
    const csvExp = useRef();
    const pdfExpFilters = {};
    var reportName = "Cost Center Heads Report"
    ...
    useEffect(async () => {
        ...
        const availCols = await FnShowCostcenterRptRecords(reportType);
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setcostcenterData(responce.content)
            } else {
                setColumns([]);
                setcostcenterData([])
            }
    ...
    const FnShowCostcenterRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'fmv_cost_center_heads_rpt'}`)
    ...
    const infoForUpdate = async (cost_center_heads_id, key) => {

        navigate("/Masters/MCostCenterHeads/FrmCostCenterHeadsEntry", { state: { costCenterID: cost_center_heads_id, keyForViewUpdate: key, compType: compType } })
    }
    function viewUpdateDelete(data, key) {
        let cost_center_heads_id = data.cost_center_heads_id
        setcostcenterId(cost_center_heads_id)
        switch (key) {
            case 'update': infoForUpdate(cost_center_heads_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(cost_center_heads_id, 'view'); break;
        }
    }
    ...
    async function DeleteCostCenter() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/FmCostCenterHeads/FnDeleteRecord/${costcenterId}/${COMPANY_ID}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("costcenter Deleted: ", responce);
            setShow(false)
    ...
    const openForm = () => {
        navigate("/Masters/MCostCenterHeads/FrmCostCenterHeadsEntry", { state: { costCenterID: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['cost_center_heads_id']
            ...
            jsonQuery['viewname']['fmv_cost_center_heads'] = 'fmv_cost_center_heads';
            jsonQuery['orderBy']['ORDER BY'] = 'cost_center_heads_id';
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Cost Center Heads</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteCostCenter} />
        </>
    )
}

export default FrmCostCenterHeadsListing