...
export default function FrmEmployeeGradeListing({ compType = 'Masters' }) {

    const configConstants = ConfigConstants();
    const { COMPANY_ID, UserName, COMPANY_NAME, USER_ACCESS } = configConstants;
    const [selectedValue, setSelectedValue] = useState("");
    const [employee_grade_id, setemployee_grade_id] = useState();
    const [employee_grade_name, setemployee_grade_name] = useState();
    ...
    // Table Data
    const [data, setDesignationData] = useState([]);
    ...
    const pdfExpFilters = {};
    var reportName = "Employee Grade Report"
    ...
    useEffect(async () => {
        ...
        const availCols = await FnShowEmployeeGradeRptRecords(reportType);
        await fetchFilteredData(PageCurrent, entriesPerPage, availCols)
    }, [compType])

    const FnShowEmployeeGradeRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_employee_grade_rpt'}`)
    ...
    const openForm = () => {
        navigate("/Masters/MEmployeeGrade/FrmEmployeeGrade", { state: { empGradeID: 0, keyForViewUpdate: 'Add', compType: compType } })
    }

    const infoForUpdate = (data, key) => {
        navigate("/Masters/MEmployeeGrade/FrmEmployeeGrade", { state: { empGradeID: data.employee_grade_id, empGradeName: data.employee_grade_name, keyForViewUpdate: key, compType: compType } })
    }


    function viewUpdateDelete(data, key) {
        setemployee_grade_id(data.employee_grade_id)
        setemployee_grade_name(data.employee_grade_name)
        switch (key) {
    ...
    async function DeleteEmployeeGradeData() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/employeegrade/FnDeleteRecord/${employee_grade_id}/${COMPANY_ID}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("Employee Grade data Deleted: ", responce);
            setShow(false)
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['employee_grade_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'employee_grade_id';
            jsonQuery['viewname']['cmv_employee_grade'] = 'cmv_employee_grade';
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setDesignationData(responce.content)
            } else {
                setColumns([]);
                setDesignationData([])
            }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Employee Grade</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteEmployeeGradeData} />
        </>
    )
}