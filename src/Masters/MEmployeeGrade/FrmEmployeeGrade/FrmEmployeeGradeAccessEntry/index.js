...
function FrmEmployeeGradeAccessEntry({ goBack, empGradeID, keyForViewUpdate, compType, empGradeName, txt_empgrade_shortName }) {
    ...
    useEffect(async () => {
        await FnEmployeeGradeWiseFormAccessData();
    }, [])

    const FnEmployeeGradeWiseFormAccessData = async () => {
        let rowCount = 0;
        ...
        const getOldAceesDataApi = await fetch(`${process.env.REACT_APP_BASE_URL}/api/employeegrade/FnShowAllActiveRecords/${parseInt(empGradeID)}/${parseInt(COMPANY_ID)}`)
        const oldAccessResponce = await getOldAceesDataApi.json();
        ...
        if (formsList.length !== 0) {
            ...
            for (let formCount = 0; formCount < formsList.length; formCount++) {
                ...
                let createTd5 = $('<div class="form-check form-switch d-flex justify-content-center align-items-center"><input class="form-check-input formAddAccessSwitch switch" type="checkbox" key="' + rowCount + '" id="addAccessSwitch-' + rowCount + '" name="addAccessSwitch-' + rowCount + '" role="switch"/></div>')
                let createTd6 = $('<div class="form-check form-switch d-flex justify-content-center align-items-center"><input class="form-check-input formReadAccessSwitch switch" type="checkbox" key="' + rowCount + '" id="readAccessSwitch-' + rowCount + '" name="readAccessSwitch-' + rowCount + '" role="switch"/></div>')
                ...
                const oldFormAccess = oldAccessResponce.filter(form => form.modules_forms_id === formsList[formCount].modules_forms_id);
                if (oldFormAccess.length > 0) {
                    $('#allAccessSwitch-' + rowCount).prop('checked', oldFormAccess[0].all_access);
                    $("#readAccessSwitch-" + rowCount).prop('checked', oldFormAccess[0].read_access);
                    $("#addAccessSwitch-" + rowCount).prop('checked', oldFormAccess[0].add_access);
                    $("#modifyAccessSwitch-" + rowCount).prop('checked', oldFormAccess[0].modify_access);
                    $("#deleteAccessSwitch-" + rowCount).prop('checked', oldFormAccess[0].delete_access);
                    $("#approvalAccessSwitch-" + rowCount).prop('checked', oldFormAccess[0].approve_access);
                    $("#specialAccessSwitch-" + rowCount).prop('checked', oldFormAccess[0].special_access);
                }
            }
            await FnManageHeadersSwitch();
    ...
    // For add designation entry.
    const addEmployeeAccessEntryData = async () => {
        try {
            setIsLoading(true)
            ...
            let DesignationAccessJson = { 'employeeGradeAcessRecords': [], 'commonIds': {} }
            for (let count = 1; count <= rowCount; count++) {
                ...
                if (addAllFormOrNot || allAccess || readAccess || addAccess || modifyAccess || deleteAccess || approvalAccess || specialAccess) {
                    ...
                    DesignationAccessJson['employeeGradeAcessRecords'].push(acceesRecord);
                }
            }

            DesignationAccessJson['commonIds']['company_id'] = parseInt(COMPANY_ID);
            DesignationAccessJson['commonIds']['company_branch_id'] = parseInt(COMPANY_BRANCH_ID);
            DesignationAccessJson['commonIds']['grade_id'] = parseInt(empGradeID);

            console.log("EmployeeGradeAccessData: ", DesignationAccessJson)
            const formData = new FormData();
            formData.append(`EmployeeGradeAccessData`, JSON.stringify(DesignationAccessJson))
            const requestOptions = {
                method: 'POST',
                body: formData
            };
            const apiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/employeegrade/FnAddUpdateGradeAccessRecord`, requestOptions)
    ...
    return (
        <>
            ...
            <div className='main_heding'>
                <label className='erp-form-label-lg main_heding'>Access Rights For Employee Grade : {empGradeName} </label>
            </div>
                ...
                <Table className="erp_table" id="FormsTbl" bordered striped>
                    <thead className='erp_table_head erp_table_th'>
                        <tr>
                            ...
                            <th className='text-center'><div class="form-switch justify-content-center"> <input class="form-check-input switch" type="checkbox" role="switch" id="allformsAllAccess" onChange={e => setPermissionsForAllForms("allformsAllAccess")} /> <label class="form-check-label ms-1" for="allformsAllAccess"> All</label></div> </th>
                            <th className='text-center'><div class="form-switch justify-content-center"> <input class="form-check-input headerSwitch  switch" type="checkbox" role="switch" id="addAllFormsAccess" onChange={e => setPermissionsForAllForms("addAllFormsAccess")} /> <label class="form-check-label ms-1" for="addAllFormsAccess"> Add</label></div> </th>
                            <th className='text-center'><div class="form-switch justify-content-center"> <input class="form-check-input headerSwitch  switch" type="checkbox" role="switch" id="readAllFormsAccess" onChange={e => setPermissionsForAllForms("readAllFormsAccess")} /> <label class="form-check-label ms-1" for="readAllFormsAccess"> Read</label></div> </th>
                            ...
                            <th className='text-center'><div class="form-switch justify-content-center"> <input class="form-check-input headerSwitch  switch" type="checkbox" role="switch" id="deleteAllFormsAccess" onChange={e => setPermissionsForAllForms("deleteAllFormsAccess")} /> <label class="form-check-label ms-1" for="deleteAllFormsAccess"> Delete</label></div> </th>
                            <th className='text-center'><div class="form-switch justify-content-center"> <input class="form-check-input headerSwitch  switch" type="checkbox" role="switch" id="approveAllFormsAccess" onChange={e => setPermissionsForAllForms("approveAllFormsAccess")} /> <label class="form-check-label ms-1" for="approveAllFormsAccess"> Approve</label></div> </th>
                            <th className='text-center'><div class="form-switch justify-content-center"> <input class="form-check-input headerSwitch  switch" type="checkbox" role="switch" id="specialAllFormsAccess" onChange={e => setPermissionsForAllForms("specialAllFormsAccess")} /> <label class="form-check-label ms-1" for="specialAllFormsAccess"> Special</label></div> </th>
                        </tr>
                    </thead>
                    <tbody className="erp_table_td"> </tbody>
                </Table>
            </div>
            <div className='text-center'>
             
                <MDButton type="button" onClick={()=>goBack(empGradeID)} className="erp-gb-button ms-2" variant="button"
                    fontWeight="regular" >Back</MDButton>
                <MDButton type="button" onClick={() => addEmployeeAccessEntryData()} className={`btn erp-gb-button ms-2 ${keyForViewUpdate === 'view' ? 'd-none' : 'display'}`} variant="button"
                    fontWeight="regular"  >Save</MDButton>
                       <MDButton type="button" className="erp-gb-button ms-2"
                    onClick={() => {
...
export default FrmEmployeeGradeAccessEntry