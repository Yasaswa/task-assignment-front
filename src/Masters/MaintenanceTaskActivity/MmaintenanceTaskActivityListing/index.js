...
function MmaintenanceTaskActivityListing({ compType = 'Masters' }) {
    ...
    const [maintenance_task_activity_id, setPropertyId] = useState();
    ...
    const [data, setMaintanceDataTaskActivity] = useState([]);
    ...
    var reportName = "Task Activity Report"
    ...
    useEffect(async () => {
        ...
        const availCols = await FnShowTaskActivityRptRecords(reportType);
        await fetchFilteredData(PageCurrent, entriesPerPage, availCols)


    }, [compType])

    const FnShowTaskActivityRptRecords = async (reportIdentifierKey) => {
        debugger
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'ymv_maintenance_task_activity_rpt'}`)
    ...
    const infoForUpdate = async (maintenance_task_activity_id, production_department_id, production_sub_department_id, key) => {
        navigate("/Masters/MaintenanceTaskActivity/MmaintenanceTaskActivityEntry", { state: { keyForViewUpdate: key, maintenance_task_activity_id: maintenance_task_activity_id, production_department_id: production_department_id, production_sub_department_id: production_sub_department_id, compType: compType } })
    }

    function viewUpdateDelete(data, key) {
        var maintenance_task_activity_id = data.maintenance_task_activity_id
        var production_department_id = data.production_department_id
        var production_sub_department_id = data.production_sub_department_id
        setPropertyId(maintenance_task_activity_id)
        switch (key) {
            case 'update': infoForUpdate(maintenance_task_activity_id, production_department_id, production_sub_department_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(maintenance_task_activity_id, production_department_id, production_sub_department_id, 'view'); break;
        }
    }


    const openForm = () => {
        navigate("/Masters/MaintenanceTaskActivity/MmaintenanceTaskActivityEntry", { state: { maintenance_task_activity_id: 0, keyForViewUpdate: '', compType: compType } })
    }

    // const openForm = () => {
    //     sessionStorage.removeItem('keyForViewUpdate');
    //     sessionStorage.removeItem('maintenance_task_activity_id');
    //     sessionStorage.removeItem('production_department_id');
    //     sessionStorage.removeItem('production_sub_department_id');
    //     navigate("/Masters/MaintenanceTaskActivity/MmaintenanceTaskActivityEntry")
    //     navigate("/Masters/MaintenanceTaskActivity/MmaintenanceTaskActivityEntry", { state: { keyForViewUpdate: '', maintenance_task_activity_id: 0, production_department_id: 0, production_sub_department_id: 0 } })
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['maintenance_task_activity_id', 'production_department_id', 'production_sub_department_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'maintenance_task_activity_id';
            jsonQuery['viewname']['viewName'] = 'ymv_maintenance_task_activity';
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setMaintanceDataTaskActivity(responce.content)
            } else {
                setColumns([]);
                setMaintanceDataTaskActivity([])
            }
            return responce;
        } catch (error) {
            console.log("error: ", error)
    ...
    async function DeleteProperty() {
        ...
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/YmMaintenanceTaskActivity/FnDeleteRecord/${maintenance_task_activity_id}/${COMPANY_ID}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("holiday record Deleted: ", responce);
            setShow(false)
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Maintenance Task Activity</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} delete={DeleteProperty} />
...
export default MmaintenanceTaskActivityListing      
