...
function FrmAgentList({ compType = 'Masters' }) {
    ...
    // Table Data
    const [data, setAgentData] = useState([]);
    ...
    useEffect(() => {
        const functionCall = async () => {
            ...
            const availCols = await FnShowAgentRptRecords(reportType);
    ...
    const FnShowAgentRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_agent_rpt'}`)
    ...
    const infoForUpdate = ({ agent_id, agent_name, agent_vendor_code }, key) => {
        navigate("/Masters/Agent", { state: { agentID: agent_id, agent_name: agent_name, agent_vendor_code: agent_vendor_code, keyForViewUpdate: key, compType: compType } })
    }
    ...
    async function DeleteAgent() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/agent/FnDeleteRecord/${agentID}/${COMPANY_ID}/${UserName}`, method)
    ...
    const openForm = () => {
        navigate("/Masters/Agent", { state: { agentID: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['agent_id']
            ...
            jsonQuery['viewname']['cmv_agent'] = 'cmv_agent';
            jsonQuery['orderBy']['ORDER BY'] = 'agent_id';
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Agent Information</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteAgent} />
        </>
    )
}
export default FrmAgentList;