...
function FrmJobTypeListing({ compType = 'Masters' }) {
    ...
    // Table Data
    const [data, setJobTypeData] = useState([]);
    ...
    const csvExp = useRef();
    const pdfExpFilters = {};
    var reportName = "Job Type Report"
    ...
    useEffect(() => {
        const functionCall = async () => {
            ...
            const availCols = await FnShowJobTypeRptRecords(reportType);
            await fetchFilteredData(PageCurrent, entriesPerPage, availCols)
        }
        functionCall()
    }, [compType])

    const FnShowJobTypeRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'hmv_job_type_rpt'}`)
    ...
    const infoForUpdate = (job_type_id, key) => {
        navigate("/Masters/MJobType/FrmJobTypeEntry", { state: { jobTypeId: job_type_id, keyForViewUpdate: key, compType: compType } })
    }

    function viewUpdateDelete(data, key) {
        var job_type_id = data.job_type_id
        setJobTypeId(job_type_id)
        switch (key) {
            case 'update': infoForUpdate(job_type_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(job_type_id, 'view'); break;
        }

    }

    async function DeleteJobType() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/jobType/FnDeleteRecord/${jobTypeId}/${UserName}`, method)
    ...
    const openForm = () => {
        navigate("/Masters/MJobType/FrmJobTypeEntry", { state: { jobTypeId: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['job_type_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'job_type_id'
            jsonQuery['viewname']['hmv_job_type'] = 'hmv_job_type';
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setJobTypeData(responce.content)
            } else {
                setColumns([]);
                setJobTypeData([])
            }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Job Type</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteJobType} />
        </>
    )
}
export default FrmJobTypeListing;