...
function FrmBankListListing({ compType = 'Masters' }) {
  ...
  //table Feilds
  const [data, setBankListData] = useState([]);
  ...
  const pdfExpFilters = {};
  var reportName = "Banklist Report"
  ...
  useEffect(async () => {
    ...
    const availCols = await FnShowBankListRptRecords(reportType);
    await fetchFilteredData(PageCurrent, entriesPerPage, availCols)
  }, [compType])


  // test case 5 and 1 shivanjali

  const FnShowBankListRptRecords = async (reportIdentifierKey) => {
    try {
      const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_banks_List_rpt'}`)
  ...
  const infoForUpdate = async (bank_id, key) => {
    navigate("/Masters/MBankList/FrmBankListEntry", { state: { bankId: bank_id, keyForViewUpdate: key, compType: compType } })
  }
  ...
  function viewUpdateDelete(data, key) {
    ...
    setbankId(bank_id)
    switch (key) {
      case 'update': infoForUpdate(bank_id, 'update'); break;
      case 'delete': setShow(true); break;
      case 'view': infoForUpdate(bank_id, 'view'); break;
    }
  }
  ...
  const openForm = () => {
    navigate("/Masters/MBankList/FrmBankListEntry", { state: { bankId: 0, keyForViewUpdate: 'Add', compType: compType } })
  }
  ...
  const handlePageClick = async (pageNo) => {
    ...
    await fetchFilteredData(currentPage, entriesPerPage);
  }
  ...
  const submitQuery = async (availCols) => {
    try {
      ...
      jsonQuery['viewname']['cmv_banks_List'] = 'cmv_banks_List';
      jsonQuery['orderBy']['ORDER BY'] = 'bank_id';
  ...
  const fetchFilteredData = async (page, size, availCols) => {
    try {
      ...
      if (responce.content.length > 0) {
        ...
        setColumns(nColumn);
        setBankListData(responce.content)
      } else {
        setColumns([]);
        setBankListData([])
      }
  ...
  async function DeleteRecord() {
    try {
      ...
      const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/CmBanksList/FnDeleteRecord/${bankId}/${COMPANY_ID}/${UserName}`, method)
      const responce = await deleteApiCall.json();
      console.log("bank record Deleted: ", responce);
      setShow(false)
    ...
    <>
      ...
      <DashboardLayout>
        <div className='main_heding'>
          <label className='erp-form-label-lg'>Bank List</label>
        </div>
      ...
      {/* Delete Modal */}
      <DeleteModal show={show} closeModal={handleClose} delete={DeleteRecord} />

    </>
  )
}

export default FrmBankListListing
