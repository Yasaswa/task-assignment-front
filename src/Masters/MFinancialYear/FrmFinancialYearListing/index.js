...
function FrmFinancialYearListing(compType = 'Masters') {
    ...
    // Table Data
    const [data, setFinancialYearData] = useState([]);
    ...
    const csvExp = useRef();
    const pdfExpFilters = {};
    var reportName = "Financial Year Report"
    ...
    useEffect(() => {
        const functionCall = async () => {
            ...
            const availCols = await FnShowFinancialYearRptRecords(reportType);
    ...
    const FnShowFinancialYearRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'amv_financial_year_rpt'}`)
    ...
    const infoForUpdate = (financial_year_id, key) => {
        navigate("/Masters/MFinancialYear/FrmFinancialYearEntry", { state: { financialYearId: financial_year_id, keyForViewUpdate: key } })
    }

    function viewUpdateDelete(data, key) {
        var financial_year_id = data.financial_year_id
        setFinancialYearId(financial_year_id)
        switch (key) {
            // case 'update': infoForUpdate(financial_year_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(financial_year_id, 'view'); break;
        }
    ...
    async function DeleteFinancialYear() {
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/finaincialyear/FnDeleteRecord/${financialYearId}/${UserName}`, method)
    ...
    const openForm = () => {
        navigate("/Masters/MFinancialYear/FrmFinancialYearEntry", { state: { financialYearId: 0, keyForViewUpdate: 'Add' } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['financial_year_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'financial_year_id'
            jsonQuery['viewname']['amv_financial_year'] = 'amv_financial_year';
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setFinancialYearData(responce.content)
            } else {
                setColumns([]);
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Financial Year</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteFinancialYear} />
        </>
    )
}
export default FrmFinancialYearListing;