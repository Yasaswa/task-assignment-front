...
export default function FrmDistrictList({ compType = 'Masters' }) {

  const configConstants = ConfigConstants();
  const { COMPANY_ID, COMPANY_NAME, UserName, USER_ACCESS } = configConstants;
  // COMPANY_NAME

  const [distId, setDistrictID] = useState();
  ...
  const csvExp = useRef();
  const pdfExpFilters = {};
  var reportName = "District Report"
  ...
  useEffect(async () => {
    ...
    const availCols = await FnShowCityRptRecords(reportType);
  ...
  const FnShowCityRptRecords = async (reportIdentifierKey) => {
    try {
      const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_district_rpt'}`)
  ...
  const infoForUpdate = async (district_id, key) => {
    navigate("/Masters/MDistrict/FrmDistrict", { state: { distId: district_id, keyForViewUpdate: key, compType: compType } })
  }
  //end chgenges
  ...
  function viewUpdateDelete(data, key) {
    ...
    switch (key) {
      case 'update': infoForUpdate(district_id, 'update'); break;
      case 'delete': setShow(true); break;
      case 'view': infoForUpdate(district_id, 'view'); break;
    }
  }

  const openForm = () => {
    navigate("/Masters/MDistrict/FrmDistrict", { state: { distId: 0, keyForViewUpdate: 'Add', compType: compType } })
  }
  ...
  async function DeleteCity() {
    ...
    try {
      ...
      const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/district/FnDeleteRecord/${distId}`, method)
      const responce = await deleteApiCall.json();
      console.log("City Deleted: ", responce);
      setShow(false)
  ...
  const submitQuery = async (availCols) => {
    try {
      const requiredColumns = ['district_id']
      ...
      jsonQuery['orderBy']['ORDER BY'] = 'district_id';
      jsonQuery['viewname']['cmv_district'] = 'cmv_district';
    ...
    <>
      ...
      <DashboardLayout>
        <div className='main_heding'>
          <label className='erp-form-label-lg'>District Information</label>
        </div>
      ...
      <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteCity} />
    </>
