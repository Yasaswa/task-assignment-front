...
function FrmMContractorList({ compType = 'Masters' }) {
  var storeSelectedValues = [];
  const [contractorID, setContractorId] = useState();

  const configConstants = ConfigConstants();
  const { COMPANY_ID, COMPANY_BRANCH_ID, UserName, COMPANY_NAME, USER_ACCESS } = configConstants;

  // Table Data
  const [data, setContractorData] = useState([]);
  ...
  const csvExp = useRef();
  const pdfExpFilters = {};
  var reportName = "Contractor Report";
  ...
  useEffect(async () => {
    ...
    const availCols = await FnShowContractorRptRecords(reportType);
  ...
  const FnShowContractorRptRecords = async (reportIdentifierKey) => {
    try {
      const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_contractor_rpt'}`)
  ...
  const submitQuery = async (availCols) => {
    try {
      const requiredColumns = ['contractor_id']
      ...
      jsonQuery['viewname']['viewName'] = 'cmv_contractor'
      jsonQuery['orderBy']['ORDER BY'] = 'contractor_id';
  ...
  const fetchFilteredData = async (page, size, availCols) => {
    try {
      ...
      if (responce.content.length > 0) {
        ...
        setColumns(nColumn);
        setContractorData(responce.content)
      } else {
        setColumns([]);
        setContractorData([])
      }
  ...
  // chnages by tushar 
  const infoForUpdate = (contractor_id, key) => {
    navigate("/Masters/Contractor", { state: { contractorID: contractor_id, keyForViewUpdate: key, compType: compType } })
    // sessionStorage.removeItem('keyForViewUpdate');
    // sessionStorage.removeItem('contractorID');
    // sessionStorage.setItem('contractorID', contractor_id);
    // sessionStorage.setItem('keyForViewUpdate', key);
    // navigate("/Masters/Contractor")
  }
  ...
  function viewUpdateDelete(data, key) {
    ...
    switch (key) {
      case 'update': infoForUpdate(contractor_id, 'update'); break;
      case 'delete': setShow(true); break;
      case 'view': infoForUpdate(contractor_id, 'view'); break;
    }
  ...
  async function DeleteContractor() {
    ...
    try {
      ...
      const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/contractor/FnDeleteRecord/${contractorID}/${UserName}`, method)
      const responce = await deleteApiCall.json();
      console.log("Cotractor Deleted: ", responce);
      setShow(false)
  ...
  // changes bu tushar
  const openForm = () => {
    navigate("/Masters/Contractor", { state: { contractorID: 0, keyForViewUpdate: 'Add', compType: compType } })
  }
    ...
    <>
      ...
      <DashboardLayout>
        <div className='main_heding'>
          <label className='erp-form-label-lg'>Contractor Information </label>
        </div>
      ...
      <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteContractor} />
    </>
  )
}
export default FrmMContractorList;
