...
function CommonParamterListing({ compType = 'Masters' }) {
    const [selectedFilters, setSelectedFilters] = useState([]);
    const [selectedValue, setSelectedValue] = useState("");
    const [commonparamterID, setcommonparamterID] = useState();
    ...
    // Table Data
    const [data, setcommonparamterData] = useState([]);
    ...
    const pdfExpFilters = {};
    var reportName = "CommonParameter Report"
    ...
    useEffect(async () => {
        ...
        const availCols = await FnShowCommonParamterRptRecords(reportType);
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setcommonparamterData(responce.content)
            } else {
                setColumns([]);
                setcommonparamterData([])
            }
            return responce;
        } catch (error) {
    ...
    const FnShowCommonParamterRptRecords = async (reportIdentifierKey) => {
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'cmv_common_parameters_rpt'}`)
    ...
    const infoForUpdate = async (common_parameters_id, key) => {
        navigate("/Masters/MCommonParameter/CommonParamterEntry", { state: { commonparamterID: common_parameters_id, keyForViewUpdate: key, compType: compType } })
    }
    function viewUpdateDelete(data, key) {

        let common_parameters_id = data.common_parameters_id
        setcommonparamterID(common_parameters_id)
        switch (key) {
            case 'update': infoForUpdate(common_parameters_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(common_parameters_id, 'view'); break;
        }

    }

    async function DeleteCommonParamter() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/CmCommonParameters/FnDeleteRecord/${commonparamterID}/${COMPANY_ID}/${UserName}`, method)
            const responce = await deleteApiCall.json();
            console.log("FmCommonParameter Deleted: ", responce);
            setShow(false)
    ...
    const openForm = () => {
        navigate("/Masters/MCommonParameter/CommonParamterEntry", { state: { commonparamterID: '', keyForViewUpdate: 0, compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['common_parameters_id']
            ...
            jsonQuery['viewname']['cmv_common_parameters'] = 'cmv_common_parameters';
            jsonQuery['orderBy']['ORDER BY'] = 'common_parameters_id';
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Common Parameter </label>
                </div>
            ...
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteCommonParamter} />

        </>
    )
}

export default CommonParamterListing