...
function ModuleFormListing(compType = 'Masters') {


    const configConstants = ConfigConstants();
    const { COMPANY_ID, COMPANY_NAME,USER_ACCESS} = configConstants;

    const [modulesformsID, setmodulesformsID] = useState();
    var storeSelectedValues = [];

    // Table Data
    const [data, setmodulesformsData] = useState([]);
    ...
    useEffect(() => {
        const functionCall = async () => {
            ...
            const availCols = await FnShowModuleFormsRptRecords(reportType);
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        ...
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setmodulesformsData(responce.content)
            } else {
                setColumns([]);
                setmodulesformsData([])
            }
    ...
    const FnShowModuleFormsRptRecords = async (reportIdentifierKey) => {
        debugger
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'amv_modules_forms_rpt'}`)
    ...
    const infoForUpdate = async (modules_forms_id, key) => {
        navigate("/Masters/ModuleFormEntry", { state: { modulesformsID: modules_forms_id, keyForViewUpdate: key, compType: compType } });
    }


    function viewUpdateDelete(data, key) {

        let modules_forms_id = data.modules_forms_id
        setmodulesformsID(modules_forms_id)
        switch (key) {
            case 'update': infoForUpdate(modules_forms_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(modules_forms_id, 'view'); break;
        }

    }

    async function DeleteModuleFormsData() {
        try {
            const method = { method: 'DELETE' }
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/AmModulesForms/FnDeleteRecord/${modulesformsID}/${COMPANY_ID}`, method)
            const responce = await deleteApiCall.json();
            console.log("FmProfitCenter Deleted: ", responce);
            setShow(false)
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['modules_forms_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'modules_forms_id';
            jsonQuery['viewname']['amv_modules_forms'] = 'amv_modules_forms';
    ...
    const openForm = () => {
        sessionStorage.removeItem('updateResponce');
        sessionStorage.removeItem('keyForViewUpdate');
        sessionStorage.removeItem('modulesformsID');
        navigate("/Masters/ModuleFormEntry", { state: { modulesformsID: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Module Form</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteModuleFormsData} />
        </>
    )
}

export default ModuleFormListing