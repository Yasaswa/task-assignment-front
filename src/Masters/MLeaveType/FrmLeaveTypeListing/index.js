...
function FrmLeaveTypeListing({ compType = 'Masters' }) {
    ...
    // Table Data
    const [data, setLeaveTypeData] = useState([]);
    ...
    const csvExp = useRef();
    const pdfExpFilters = {};
    var reportName = "Leave Type Report"
    ...
    useEffect(() => {
        const functionCall = async () => {
            ...
            const availCols = await FnShowLeaveTypeRptRecords(reportType);
    ...
    const FnShowLeaveTypeRptRecords = async (reportIdentifierKey) => {
        debugger
        try {
            const summaryRptApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/report/FnShowAllReportRecords?reportType=${'summary'}&viewName=${'hmv_leave_type_rpt'}`)
    ...
    const infoForUpdate = (leave_type_id, key) => {
        navigate("/Masters/MLeaveType/FrmLeaveTypeEntry", { state: { leaveTypeId: leave_type_id, keyForViewUpdate: key, compType: compType } })
    }
    ...
    function viewUpdateDelete(data, key) {
        ...
        switch (key) {
            case 'update': infoForUpdate(leave_type_id, 'update'); break;
            case 'delete': setShow(true); break;
            case 'view': infoForUpdate(leave_type_id, 'view'); break;
        }
    ...
    async function DeleteLeaveType() {
        ...
        try {
            ...
            const deleteApiCall = await fetch(`${process.env.REACT_APP_BASE_URL}/api/HmLeaveType/FnDeleteRecord/${leaveTypeId}/${UserName}`, method)
    ...
    const openForm = () => {
        navigate("/Masters/MLeaveType/FrmLeaveTypeEntry", { state: { leaveTypeId: 0, keyForViewUpdate: 'Add', compType: compType } })
    }
    ...
    const submitQuery = async (availCols) => {
        try {
            const requiredColumns = ['leave_type_id']
            ...
            jsonQuery['orderBy']['ORDER BY'] = 'leave_type_id'
            jsonQuery['viewname']['hmv_leave_type'] = 'hmv_leave_type';
    ...
    const fetchFilteredData = async (page, size, availCols) => {
        try {
            ...
            if (responce.content.length > 0) {
                ...
                setColumns(nColumn);
                setLeaveTypeData(responce.content)
            } else {
                setColumns([]);
                setLeaveTypeData([])
            }
        ...
        <>
            ...
            <DashboardLayout>
                <div className='main_heding'>
                    <label className='erp-form-label-lg'>Leave Type Details</label>
                </div>
            ...
            {/* Delete Modal */}
            <DeleteModal show={show} closeModal={handleClose} deleteRecord={DeleteLeaveType} />

        </>

    )
}
export default FrmLeaveTypeListing;